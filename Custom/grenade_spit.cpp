...
LINK_ENTITY_TO_CUSTOM_CLASS(grenade_spit, grenade, CGrenadeSpit);
...
void CGrenadeSpit::Spawn( void )
{
	...
	UTIL_SetSize( BaseEntity(), -Vector(16,16,16), Vector(16,16,16) );

	SetUse( &CE_Grenade::DetonateUse );
	...
	CEntity *_m_hSpitEffect = (CEntity *) CreateEntityByName( "info_particle_system" );
	if ( _m_hSpitEffect != NULL )
	{
...
//-----------------------------------------------------------------------------
void CGrenadeSpit::GrenadeSpitTouch( CEntity *pOther )
{
	...
	const trace_t *pTrace = CEntity::GetTouchTrace();
	...
	CEntity *const pTraceEnt = CEntity::Instance(pTrace->m_pEnt);
	...
	CEntity *thrower = GetThrower();
	if ( pTraceEnt )
	{
		pTraceEnt->TakeDamage( CTakeDamageInfo( BaseEntity(), (thrower)?thrower->BaseEntity():NULL, m_flDamage * (1.0f-poisonratio), DMG_ACID ) );
		pTraceEnt->TakeDamage( CTakeDamageInfo( BaseEntity(), (thrower)?thrower->BaseEntity():NULL, m_flDamage * poisonratio, DMG_POISON ) );
	}

	g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, GetAbsOrigin(), (int)(m_DmgRadius * 2.0f), 0.5f, (thrower)?thrower->BaseEntity():NULL );
	...
	if ( pOther->IsPlayer() || bHitWater )
	{
		...
		DispatchParticleEffect( "blood_impact_antlion_01", GetAbsOrigin(), vecAngles );
	}
	else
	{
		DispatchParticleEffect( "blood_impact_antlion_01", GetAbsOrigin(), vecAngles );
...
void CGrenadeSpit::Detonate(void)
{
	...
	{
		g_SoundController->SoundDestroy( m_pHissSound );
		m_pHissSound = NULL;
	}
...
void CGrenadeSpit::InitHissSound( void )
{
	...
	if ( m_pHissSound == NULL )
	{
		...
		m_pHissSound = g_SoundController->SoundCreate( filter, entindex(), "NPC_Antlion.PoisonBall" );
		g_SoundController->Play( m_pHissSound, 1.0f, 100 );
	}
}
...
void CGrenadeSpit::Think( void )
{
	...
	if ( pPlayer != NULL )
	{
		...
		g_SoundController->SoundChangePitch( m_pHissSound, iPitch, 0.1f );
	}
