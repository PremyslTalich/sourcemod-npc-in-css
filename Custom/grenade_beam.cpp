...
LINK_ENTITY_TO_CUSTOM_CLASS( grenade_beam_chaser, item_sodacan, CGrenadeBeamChaser );
...
LINK_ENTITY_TO_CUSTOM_CLASS( grenade_beam, grenade, CGrenadeBeam );
...
CGrenadeBeam::CGrenadeBeam()
{
	...
	m_flBeamLag = 0.0f;
	m_flLaunchTime = 0.0f;
	m_flLastTouchTime = 0.0f;
	m_nNumHits = 0;
	memset(m_pHitLocation,0,sizeof(m_pHitLocation));
...
// Output  :
//------------------------------------------------------------------------------
CGrenadeBeam* CGrenadeBeam::Create( CEntity* pOwner, const Vector &vStart)
{
	CGrenadeBeam *pEnergy = (CGrenadeBeam *)CreateEntityByName( "grenade_beam" );
	pEnergy->Spawn();
	pEnergy->SetOwnerEntity( (pOwner)?pOwner->BaseEntity():NULL );
...
void CGrenadeBeam::Shoot(Vector vDirection, float flSpeed, float flLifetime, float flLag, float flDamage )
{
	...
	CGrenadeBeamChaser *pBeam = CGrenadeBeamChaser::ChaserCreate(this);

	m_hBeamChaser.Set((pBeam)?pBeam->BaseEntity():NULL);
...
// Output  :
//------------------------------------------------------------------------------
void CGrenadeBeam::GrenadeBeamTouch( CEntity *pOther )
{
	//---------------------------------------------------------
	...
	{
		pOther->TakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), m_flDamage, DMG_BURN ) );
		KillBeam();
		return;
	}
...
void CGrenadeBeam::CreateBeams(void)
{
	for ( int i=0; i < GRENADEBEAM_MAXBEAMS; ++i )
	{
		m_pBeam[i] = CE_CBeam::BeamCreate( "sprites/laser.vmt", m_flBeamWidth );
