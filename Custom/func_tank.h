...
class CFuncTank : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CFuncTank, CE_InfoTarget_Fix );
	DECLARE_DATADESC();
...
public:
	...
	void	NPC_JustSawPlayer( CEntity *pTarget );
	void	NPC_SetInRoute( bool bInRoute )				{ m_bNPCInRoute = bInRoute; }
	void	NPC_SetIdleAngle( Vector vecIdle )			{ m_vecNPCIdleTarget = vecIdle; }

	// LOS
	bool	IsEntityInViewCone( CEntity *pEntity );
	bool	HasLOSTo( CEntity *pEntity );

	// Controller
	CCombatCharacter *GetController( void );
	bool StartControl( CCombatCharacter *pController );
...
protected:
	...
	virtual void Fire( int bulletCount, const Vector &barrelEnd, const Vector &forward, CEntity *pAttacker, bool bIgnoreSpread );
...
private:
	...
	virtual void FiringSequence( const Vector &barrelEnd, const Vector &forward, CEntity *pAttacker );
...
private:
	...
	CEntity *FindTarget( string_t targetName, CEntity *pActivator );
...
private:
	...
	CEFakeHandle<CCombatCharacter> m_hController;
	...
	CEFakeHandle<CTrigger>	m_hControlVolume;
