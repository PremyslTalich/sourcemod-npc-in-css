...
CEnvHeadcrabCanisterShared::CEnvHeadcrabCanisterShared()
{
	...
	m_vecEnterWorldPosition.Init();
	m_vecStartAngles.Init();

	m_flInitialZSpeed = 0.0f;
	m_flZAcceleration = 0.0f;
	m_flHorizSpeed = 0.0f;

	m_bLaunchedFromWithinWorld = false;

	m_vecParabolaDirection.Init();

	// Skybox data
	m_vecSkyboxOrigin.Init();
	m_flSkyboxScale = 0.0f;

	m_flLaunchHeight = 0.0f;
}
	...
	const Vector &vecDirection, const Vector &vecImpactPosition, bool bLaunchedFromWithinWorld )
{
	...
	if ( m_bLaunchedFromWithinWorld )
	{
		...
		float flLength = m_vecDirection.AsVector2D().Length();
		VectorSubtract(vecImpactPosition, vecStartPosition, m_vecParabolaDirection);
		m_vecParabolaDirection.z = 0;
		float flTotalDistance = VectorNormalize( m_vecParabolaDirection );
		m_vecDirection.x = flLength * m_vecParabolaDirection.x;
		m_vecDirection.y = flLength * m_vecParabolaDirection.y;
 
		m_flHorizSpeed = flTotalDistance / m_flFlightTime;
		m_flWorldEnterTime = 0;
 
		float flFinalZSpeed = m_vecDirection.z * m_flHorizSpeed;
	...
	const Vector &vecImpactPosition, const Vector &vecSkyboxOrigin, float flSkyboxScale )
{
	...
	float flLength = VectorNormalize( m_vecParabolaDirection ); 
	...
	VectorMA( vecSkyboxOrigin, 1.0f / m_flSkyboxScale, vecStartPosition, m_vecStartPosition );
	VectorMA( m_vecStartPosition, -m_flHorizSpeed * m_flWorldEnterTime, m_vecParabolaDirection, m_vecStartPosition );

	float flLaunchHeight = m_flLaunchHeight / flSkyboxScale;
	float flFinalZSpeed = m_vecDirection.z * m_flFlightSpeed / flSkyboxScale;
	m_vecStartPosition.z += flLaunchHeight;
...
void CEnvHeadcrabCanisterShared::GetPositionAtTime( float flTime, Vector &vecPosition, QAngle &vecAngles )
{
	...
	else
	{
		...
		MatrixFromAngles( m_vecStartAngles, initToWorld );
	}
