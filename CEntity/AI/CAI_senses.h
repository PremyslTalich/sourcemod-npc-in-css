...
public:
	...
	bool			DidSeeEntity( CEntity *pSightEnt ) const; //  a less expensive query that looks at cached results from recent conditionsa gathering

	CEntity *		GetFirstSeenEntity( AISightIter_t *pIter, seentype_t iSeenType = SEEN_ALL ) const;
	CEntity *		GetNextSeenEntity( AISightIter_t *pIter ) const;
