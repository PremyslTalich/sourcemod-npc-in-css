...
DEFINE_PROP(m_OnArrival, CE_CRallyPoint);
DEFINE_PROP(m_RallySequenceName, CE_CRallyPoint);
DEFINE_PROP(m_hLockedBy, CE_CRallyPoint);
DEFINE_PROP(m_iPriority, CE_CRallyPoint);
DEFINE_PROP(m_bForceCrouch, CE_CRallyPoint);
DEFINE_PROP(m_bIsUrgent, CE_CRallyPoint);
DEFINE_PROP(m_AssaultPointName, CE_CRallyPoint);
DEFINE_PROP(m_sExclusivity, CE_CRallyPoint);



CE_LINK_ENTITY_TO_CLASS(CAssaultPoint, CE_CAssaultPoint);

DEFINE_PROP(m_flTimeLastUsed, CE_CAssaultPoint);
DEFINE_PROP(m_NextAssaultPointName, CE_CAssaultPoint);
DEFINE_PROP(m_AssaultHintGroup, CE_CAssaultPoint);
DEFINE_PROP(m_OnAssaultClear, CE_CAssaultPoint);
DEFINE_PROP(m_OnArrival, CE_CAssaultPoint);
DEFINE_PROP(m_bNeverTimeout, CE_CAssaultPoint);
DEFINE_PROP(m_flAssaultTimeout, CE_CAssaultPoint);
DEFINE_PROP(m_bInputForcedClear, CE_CAssaultPoint);
DEFINE_PROP(m_bClearOnContact, CE_CAssaultPoint);
DEFINE_PROP(m_flAssaultPointTolerance, CE_CAssaultPoint);
DEFINE_PROP(m_iStrictness, CE_CAssaultPoint);
DEFINE_PROP(m_bForceCrouch, CE_CAssaultPoint);
DEFINE_PROP(m_bIsUrgent, CE_CAssaultPoint);
DEFINE_PROP(m_bAllowDiversion, CE_CAssaultPoint);
DEFINE_PROP(m_flAllowDiversionRadius, CE_CAssaultPoint);
...
//---------------------------------------------------------
bool CE_CRallyPoint::IsExclusive()
{
...
CAI_AssaultBehavior::CAI_AssaultBehavior()
{
	...
	m_bDiverting = false;
	m_flLastSawAnEnemyAt = 0.0f;
	m_flTimeDeferScheduleSelection = 0.0f;
...
//-----------------------------------------------------------------------------
CE_CAssaultPoint *CAI_AssaultBehavior::FindAssaultPoint( string_t iszAssaultPointName )
{
	CUtlVector<CE_CAssaultPoint*>pAssaultPoints;
	CUtlVector<CE_CAssaultPoint*>pClearAssaultPoints;

	CE_CAssaultPoint *pAssaultEnt = (CE_CAssaultPoint *)g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, iszAssaultPointName );

	while( pAssaultEnt != NULL )
	{
		pAssaultPoints.AddToTail( pAssaultEnt );
		pAssaultEnt = (CE_CAssaultPoint *)g_helpfunc.FindEntityByName( pAssaultEnt, iszAssaultPointName );
	}
	...
	{
		trace_t tr;
		CAI_NPC *pNPC = GetOuter();
		CE_CAssaultPoint *pAssaultPoint = pAssaultPoints[i];

		UTIL_TraceHull ( pAssaultPoint->GetAbsOrigin(), pAssaultPoint->GetAbsOrigin(), pNPC->WorldAlignMins(), pNPC->WorldAlignMaxs(), MASK_SOLID, pNPC->BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	if( pClearAssaultPoints.Count() < 1 )
		return pAssaultPoints[ enginerandom->RandomInt(0, (pAssaultPoints.Count() - 1)) ];
	...
	return pClearAssaultPoints[ enginerandom->RandomInt(0, (pClearAssaultPoints.Count() - 1)) ];
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_AssaultBehavior::SetAssaultPoint( CE_CAssaultPoint *pAssaultPoint )
{
	m_hAssaultPoint.Set(pAssaultPoint->BaseEntity());
...
void CAI_AssaultBehavior::ClearAssaultPoint( void )
{
	...
	{
		CE_CAssaultPoint *pNextPoint = FindAssaultPoint( m_hAssaultPoint->m_NextAssaultPointName );
	...
	if( IsCurSchedule( SCHED_MOVE_TO_ASSAULT_POINT ) && hl2_episodic->GetBool() )
	{
		if( m_hAssaultPoint && m_hAssaultPoint->HasSpawnFlags(SF_ASSAULTPOINT_CLEARONARRIVAL) && *(m_hAssaultPoint->m_NextAssaultPointName) != NULL_STRING )
		{
			...
			if( flDist <= GetOuter()->GetMotor()->MinStoppingDist() )
			{
				...
				goal.pTarget = m_hAssaultPoint->BaseEntity();
	...
	case TASK_GET_PATH_TO_RALLY_POINT:
		{
			...
			goal.pTarget = m_hRallyPoint->BaseEntity();
			if ( GetNavigator()->SetGoal( goal ) == false )
			{
...
void CAI_AssaultBehavior::RunTask( const Task_t *pTask )
{
	...
	case TASK_WAIT_FOR_MOVEMENT:
		...
		{
			CEntity *pNewEnemy = CEntity::Instance(GetOuter()->BestEnemy());

			if( pNewEnemy != NULL && pNewEnemy != GetOuter()->GetEnemy() )
			{
				GetOuter()->SetEnemy( pNewEnemy->BaseEntity() );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CE_CRallyPoint *CAI_AssaultBehavior::FindBestRallyPointInRadius( const Vector &vecCenter, float flRadius )
{
	const int RALLY_SEARCH_ENTS	= 30;
	CEntity *pEntities[RALLY_SEARCH_ENTS];
	int iNumEntities = UTIL_EntitiesInSphere( pEntities, RALLY_SEARCH_ENTS, vecCenter, flRadius, 0 );

	CE_CRallyPoint *pBest = NULL;
	int iBestPriority = -1;

	for ( int i = 0; i < iNumEntities; i++ )
	{
		CE_CRallyPoint *pRallyEnt = dynamic_cast<CE_CRallyPoint *>(pEntities[i]);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_AssaultBehavior::IsValidShootPosition( const Vector &vLocation, CAI_Node *pNode, CBaseEntity const *pHint )
{
	CEntity *pCuePoint = NULL;
...
void CAI_AssaultBehavior::SetParameters( CBaseEntity *pRallyEnt, AssaultCue_t assaultcue )
{
	...
	CEntity *cent = CEntity::Instance(pRallyEnt);

	// Firstly, find a rally point. 
	CE_CRallyPoint *pRallyPoint = dynamic_cast<CE_CRallyPoint *>(cent);
...
void CAI_AssaultBehavior::SetParameters( string_t rallypointname, AssaultCue_t assaultcue, int rallySelectMethod )
{
	...
	// Firstly, find a rally point. 
	CE_CRallyPoint *pRallyEnt = dynamic_cast<CE_CRallyPoint *>(g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, rallypointname ) );
	...
	case RALLY_POINT_SELECT_DEFAULT:
		{
			while( pRallyEnt )
			{
				...
				pRallyEnt = dynamic_cast<CE_CRallyPoint *>(g_helpfunc.FindEntityByName( pRallyEnt, rallypointname, NULL ) );
			}
		}
		break;
	...
	case RALLY_POINT_SELECT_RANDOM:
		{
			...
			CUtlVector<CE_CRallyPoint *> rallyPoints; // List of rally points that are available to choose from.
			...
			while( pRallyEnt )
			{
				...
				pRallyEnt = dynamic_cast<CE_CRallyPoint *>(g_helpfunc.FindEntityByName( pRallyEnt, rallypointname ) );
			}

			if( rallyPoints.Count() > 0 )
			{
				pBest = rallyPoints[ enginerandom->RandomInt(0, rallyPoints.Count()- 1) ];
			}
		}
		break;
	...
	pBest->Lock( GetOuter() );
	m_hRallyPoint.Set(pBest->BaseEntity());
...
void CAI_AssaultBehavior::InitializeBehavior()
{
	...
	CE_CAssaultPoint *pAssaultEnt = FindAssaultPoint( m_hRallyPoint->m_AssaultPointName );
...
bool CAI_AssaultBehavior::IsAllowedToDivert( void )
{
	...
	{
		float value = m_hAssaultPoint->m_flAllowDiversionRadius;
		if ( value == 0.0f || (m_bHitAssaultPoint && GetEnemy() != NULL && GetEnemy()->GetAbsOrigin().DistToSqr(m_hAssaultPoint->GetAbsOrigin()) <= Square(value)) ) 
		{
