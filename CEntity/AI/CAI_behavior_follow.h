...
class CE_AI_FollowGoal : public CE_AI_GoalEntity
{
public:
	CE_DECLARE_CLASS( CE_AI_FollowGoal, CE_AI_GoalEntity );

	DECLARE_DATAMAP(int, m_iFormation);
...
int AIGetNumFollowers( CEntity *pEntity, string_t iszClassname = NULL_STRING );
...
public:
	...
	CEntity		*	GetFollowTarget();
	void			SetFollowTarget( CEntity *pLeader, bool fFinishCurSchedule = false );

	CE_AI_FollowGoal *GetFollowGoal()	{ return m_hFollowGoalEnt; } // if any
	bool			SetFollowGoal( CE_AI_FollowGoal *pGoal, bool fFinishCurSchedule = false );
	void			ClearFollowGoal( CE_AI_FollowGoal *pGoal );
	void			SetFollowGoalDirect( CE_AI_FollowGoal *pGoal );
...
protected:
	...
	bool			HasFollowPoint();
	void 			SetFollowPoint( CE_AI_Hint *pHintNode );
	void			ClearFollowPoint();
	const Vector &	GetFollowPoint();
	CE_AI_Hint *	FindFollowPoint();
...
protected:
	...
	CE_AI_Hint *					m_pInterruptWaitPoint;
