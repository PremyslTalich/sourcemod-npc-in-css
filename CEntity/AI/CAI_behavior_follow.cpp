...
//Datamaps
DEFINE_PROP(m_iFormation, CE_AI_FollowGoal);



extern ConVar *ai_follow_use_points;
extern ConVar *ai_follow_use_points_when_moving;
...
public:
	...
	bool AddFollower( CEntity *pTarget, CAI_NPC *pFollower, AI_Formations_t formation, AI_FollowManagerInfoHandle_t *pHandle );
	...
	int CountFollowersInGroup( CAI_NPC *pMember )
	{
	...
	int CountFollowers( CEntity *pFollowTarget, string_t iszClassname )
	{
	...
	int GetFollowerSlot( CAI_NPC *pFollower )
	{
...
private:
	...
	AI_FollowGroup_t *FindCreateGroup( CEntity *pTarget, AI_Formations_t formation );
	AI_FollowGroup_t *FindGroup( CEntity *pTarget );
	AI_FollowGroup_t *FindFollowerGroup( CEntity *pFollower );
...
int AIGetNumFollowers( CEntity *pEntity, string_t iszClassname )
{
...
CEntity * CAI_FollowBehavior::GetFollowTarget()
{ 
...
void CAI_FollowBehavior::SetFollowTarget( CEntity *pLeader, bool fFinishCurSchedule ) 
{ 
	...
	if ( pLeader ) 
	{
		...
		{
			m_hFollowTarget.Set(pLeader->BaseEntity());
			m_bFirstFacing = true;
...
bool CAI_FollowBehavior::SetFollowGoal( CE_AI_FollowGoal *pGoal, bool fFinishCurSchedule )
{
...
void CAI_FollowBehavior::ClearFollowGoal( CE_AI_FollowGoal *pGoal )
{
...
bool CAI_FollowBehavior::UpdateFollowPosition()
{
	...
	CEntity *pFollowTarget = GetFollowTarget();
...
bool CAI_FollowBehavior::IsFollowGoalInRange( float tolerance, float zTolerance, int flags )
{
	...
	if ( flags & AIFF_REQUIRE_LOS_OUTSIDE_COMBAT && m_hFollowTarget )
	{
...
{
	CEntity *cent = GetOuter()->m_hCine;
	CEAI_ScriptedSequence *ptr = (CEAI_ScriptedSequence *)cent;
	if ( ptr )
		ptr->CancelScript();
...
void CAI_FollowBehavior::GatherConditions( void )
{
	...
	if ( (GetFollowTarget()->IsPlayer() && HasCondition( COND_SEE_PLAYER )) || GetOuter()->FVisible_Entity( GetFollowTarget()->BaseEntity()) )
	{
...
CE_AI_Hint *CAI_FollowBehavior::FindFollowPoint()
{
...
void CAI_FollowBehavior::SetFollowPoint( CE_AI_Hint *pHintNode )
{
	if ( !pHintNode )
		return;
	...
	if ( !pHintNode->Lock( GetOuter()->BaseEntity() ) )
	{
		SetHintNode( NULL );
...
int CAI_FollowBehavior::SelectScheduleMoveToFormation()
{
		...
		!IsFollowGoalInRange( GetGoalRange(), GetGoalZRange(), GetGoalFlags() ) )
	{
		...
		if ( pSquad )
		{
			for ( CBaseEntity *pSquadMember = pSquad->GetFirstMember( &iter ); pSquadMember; pSquadMember = pSquad->GetNextMember( &iter ) )
			{
				CAI_NPC *npc = (CAI_NPC *)CEntity::Instance(pSquadMember);
				if ( npc->HasCondition( COND_PLAYER_PUSHING ) )
				{
					return SCHED_NONE;
				}
			}
...
{
	//if ( !dynamic_cast<CPointEntity *>(m_hFollowTarget) )
	if ( m_hFollowTarget->MyCombatCharacterPointer())
	{
		trace_t tr;
		...
		pPlayer = dynamic_cast<CPlayer *>(m_hFollowTarget.Get());
		...
		AI_TraceLOS( vecStart, vecStart + vecDir * 8192, m_hFollowTarget->BaseEntity(), &tr );
...
bool CAI_FollowBehavior::ValidateFaceTarget( Vector *pFaceTarget )
{
	...
	trace_t tr;
	UTIL_TraceLine( GetOuter()->EyePosition(), testPoint, MASK_BLOCKLOS, (m_hFollowTarget)?m_hFollowTarget->BaseEntity():NULL, COLLISION_GROUP_NONE, &tr );
...
bool CAI_FollowBehavior::FindCoverFromEnemyAtFollowTarget( float coverRadius, Vector *pResult )
{
	...
	return GetOuter()->FindCoverPosInRadius( (pEntity)?pEntity->BaseEntity():NULL, m_FollowNavGoal.position, coverRadius, pResult );
}
...
void CAI_FollowBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_FACE_FOLLOW_TARGET:
		{
			...
			//bool bFollowingPoint = ( dynamic_cast<CPointEntity *>(m_hFollowTarget) != NULL );
			bool bFollowingPoint = ( m_hFollowTarget->MyCombatCharacterPointer() == NULL );
			if ( GetNpcState() == NPC_STATE_COMBAT )
			{
		...
		{
			CEntity *pLeader = GetFollowTarget();
			if ( pLeader )
			{
			...
			case 2:
				{
					...
					{
						goal.pTarget = m_hFollowTarget->BaseEntity();
					}
					else
					{
						goal.pTarget = m_hFollowTarget->GetParent()->BaseEntity();
					}
			...
			else
			{
				...
				else
				{
					...
					if ( GetNavigator()->GetGoalPos() != vFollowPoint )
					{
						...
						{
							Activity hintActivity = (Activity)CAI_NPC::GetActivityID( STRING(GetHintNode()->HintActivityName()) );
...
Activity CAI_FollowBehavior::NPC_TranslateActivity( Activity activity )
{
	...
	{
		return GetOuter()->GetHintActivity(GetHintNode()->HintType(), (Activity)CAI_NPC::GetActivityID( STRING(GetHintNode()->HintActivityName()) ) );
	}
...
bool CAI_FollowBehavior::FValidateHintType( CBaseEntity *cbase )
{
	CE_AI_Hint *pHint = (CE_AI_Hint *)CEntity::Instance(cbase);
	if ( pHint->HintType() == HINT_FOLLOW_WAIT_POINT )
	{
		if ( GetFollowTarget() && GetFollowTarget()->FVisible_Vector( pHint->GetAbsOrigin() + Vector( 0, 0, 0.1 ) )  )
			return true;
		else
			return false;
	}
	return BaseClass::FValidateHintType( cbase );
}
...
bool CAI_FollowManager::AddFollower( CEntity *pTarget, CAI_NPC *pFollower, AI_Formations_t formation, AI_FollowManagerInfoHandle_t *pHandle )
{
...
bool CAI_FollowManager::RedistributeSlots( AI_FollowGroup_t *pGroup )
{
	...
	CUtlRBTree<CEntity *> movedFollowers;
...
AI_FollowGroup_t *CAI_FollowManager::FindCreateGroup( CEntity *pTarget, AI_Formations_t formation )
{
	...
	if ( !pGroup )
	{
		...
		memset(pGroup,0,sizeof(AI_FollowGroup_t));
		}
...
AI_FollowGroup_t *CAI_FollowManager::FindGroup( CEntity *pTarget )
{
	for ( int i = 0; i < m_groups.Count(); i++ )
	{
...
AI_FollowGroup_t *CAI_FollowManager::FindFollowerGroup( CEntity *pFollower )
{
	for ( int i = 0; i < m_groups.Count(); i++ )
	{
