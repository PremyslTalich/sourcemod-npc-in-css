...
//=========================================================
//=========================================================
class CAI_Relationship : public CE_InfoTarget_Fix, public IEntityListener
{
public:
	CE_DECLARE_CLASS( CAI_Relationship, CE_InfoTarget_Fix );

	CAI_Relationship()
	{
		m_iPreviousDisposition = -1;
		m_iszSubject = NULL_STRING;
		m_iPreviousRank = 0;
	}
	...
	void ChangeRelationships( int disposition, int iReverting, CEntity *pActivator = NULL, CEntity *pCaller = NULL );

	void ApplyRelationship( CEntity *pActivator = NULL, CEntity *pCaller = NULL );
	void RevertRelationship( CEntity *pActivator = NULL, CEntity *pCaller = NULL );
	void RevertToDefaultRelationship( CEntity *pActivator = NULL, CEntity *pCaller = NULL );

	void UpdateOnRemove();
	void OnRestore();

	bool IsASubject( CEntity *pEntity );
	bool IsATarget( CEntity *pEntity );
...
private:
	...
	CEntity *FindEntityForProceduralName( string_t iszName, CEntity *pActivator, CEntity *pCaller );
	void	DiscloseNPCLocation( CCombatCharacter *pSubject, CCombatCharacter *pTarget );
...
LINK_ENTITY_TO_CUSTOM_CLASS( ai_relationship, info_target, CAI_Relationship );
...
{
	ApplyRelationship( CEntity::Instance(inputdata.pActivator), CEntity::Instance(inputdata.pCaller) );
}
...
{
	RevertRelationship( CEntity::Instance(inputdata.pActivator), CEntity::Instance(inputdata.pCaller) );
}
...
{
	RevertToDefaultRelationship( CEntity::Instance(inputdata.pActivator), CEntity::Instance(inputdata.pCaller) );
}
...
//---------------------------------------------------------
void CAI_Relationship::ApplyRelationship( CEntity *pActivator, CEntity *pCaller )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::RevertRelationship( CEntity *pActivator, CEntity *pCaller )
{
	if ( m_bIsActive )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::RevertToDefaultRelationship( CEntity *pActivator, CEntity *pCaller )
{
	if ( m_bIsActive )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CAI_Relationship::IsASubject( CEntity *pEntity )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CAI_Relationship::IsATarget( CEntity *pEntity )
{
...
void CAI_Relationship::OnEntitySpawned( CBaseEntity *pEntity )
{
	...
	if ( IsATarget( cent ) || IsASubject( cent ) )
	{
		ApplyRelationship();
	}
}
...
CEntity *CAI_Relationship::FindEntityForProceduralName( string_t iszName, CEntity *pActivator, CEntity *pCaller )
{
...
//-----------------------------------------------------------------------------
void CAI_Relationship::DiscloseNPCLocation( CCombatCharacter *pSubject, CCombatCharacter *pTarget )
{
	if ( pSubject == NULL || pTarget == NULL )
		return;

	CAI_NPC *pNPC = pSubject->MyNPCPointer();
	if ( pNPC != NULL )
	{
		pNPC->UpdateEnemyMemory( pTarget->BaseEntity(), pTarget->GetAbsOrigin() );
	}
}

//---------------------------------------------------------
//---------------------------------------------------------
void CAI_Relationship::ChangeRelationships( int disposition, int iReverting, CEntity *pActivator, CEntity *pCaller )
{
	...
	CUtlVectorFixed<CCombatCharacter *, MAX_HANDLED> subjectList;
	CUtlVectorFixed<CCombatCharacter *, MAX_HANDLED> targetList;

	// Add any special subjects we found
	CEntity *pSpecialSubject = FindEntityForProceduralName( m_iszSubject, pActivator, pCaller );
	...
	CEntity *pSpecialTarget = FindEntityForProceduralName( m_target, pActivator, pCaller );
	...
	{
		CCombatCharacter *pSubject = subjectList[ i ];

		for ( int j = 0 ; j < targetList.Count(); j++ )
		{
			CCombatCharacter *pTarget = targetList[ j ];

			if ( m_iPreviousDisposition == -1 && iReverting == NOT_REVERTING )
			{
				// Set previous disposition.
				m_iPreviousDisposition = pSubject->IRelationType( pTarget->BaseEntity() );
				m_iPreviousRank = pSubject->IRelationPriority( pTarget->BaseEntity() );
			}

			if ( iReverting == REVERTING_TO_PREV )
			{
				pSubject->AddEntityRelationship( pTarget->BaseEntity(), (Disposition_t)m_iPreviousDisposition, m_iPreviousRank );

				if( m_bReciprocal )
				{
					pTarget->AddEntityRelationship( pSubject->BaseEntity(), (Disposition_t)m_iPreviousDisposition, m_iPreviousRank );
				}
			}
			else if ( iReverting == REVERTING_TO_DEFAULT )
			{
				pSubject->RemoveEntityRelationship( pTarget->BaseEntity() );

				if( m_bReciprocal )
				{
					pTarget->RemoveEntityRelationship( pSubject->BaseEntity() );
				}
			}
			else if( pSubject->IRelationType(pTarget->BaseEntity()) != disposition || 
				     pSubject->IRelationPriority(pTarget->BaseEntity()) != m_iRank || 
					 ...
					 HasSpawnFlags( SF_RELATIONSHIP_NOTIFY_TARGET ) )
			{
				...
				pSubject->AddEntityRelationship( pTarget->BaseEntity(), (Disposition_t)disposition, m_iRank );
				...
				if ( m_bReciprocal )
				{
					...
					pTarget->AddEntityRelationship( pSubject->BaseEntity(), (Disposition_t)disposition, m_iRank );
				}
			}
		}
	}
