...
public:
	...
	{ 
		if ( maxInterval > 0.0 )
			m_next = gpGlobals->curtime + enginerandom->RandomFloat( minInterval, maxInterval );
		else
	...
	{ 
		...
		else
		{
			if ( m_maxInterval == 0 )
				...
				m_next = gpGlobals->curtime + enginerandom->RandomFloat( m_minInterval, m_maxInterval );
		}
	}
...
public:
	...
	{ 
		...
		if ( maxOverride == 0 )
			...
			m_next = gpGlobals->curtime + enginerandom->RandomFloat( minOverride, maxOverride );
	}
