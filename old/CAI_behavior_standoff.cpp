...
class CE_CAI_BattleLine : public CEntity
{
public:
	CE_DECLARE_CLASS( CE_CAI_BattleLine, CEntity );

public:
	bool Affects( CAI_NPC *pNpc )
	{
		const char *pszNamedActor = STRING( (*(m_iszActor)) );
...
public: //Datamaps
	DECLARE_DATAMAP(bool, m_fActive);
	DECLARE_DATAMAP(string_t, m_iszActor);
	DECLARE_DATAMAP(bool, m_fStrict);



};

CE_LINK_ENTITY_TO_CLASS(CAI_BattleLine, CE_CAI_BattleLine);


//Datamaps
DEFINE_PROP(m_fActive, CE_CAI_BattleLine);
DEFINE_PROP(m_iszActor, CE_CAI_BattleLine);
DEFINE_PROP(m_fStrict, CE_CAI_BattleLine);
 ...
 :	CAI_MappedActivityBehavior_Temporary( pOuter )
{
	...
	m_fTestNoDamage = false;
	m_posture = AIP_INDIFFERENT;
	m_fTakeCover = false;
	m_SavedDistTooFar = 0.0f;
	m_nSavedMinShots = 0;
	m_nSavedMaxShots = 0;
	m_flSavedMinRest = 0.0f;
	m_flSavedMaxRest = 0.0f;
...
void CAI_StandoffBehavior::SetParameters( const AI_StandoffParams_t &params, CE_AI_GoalEntity *pGoalEntity )
{
	m_params = params;
	m_hStandoffGoal = (pGoalEntity)?pGoalEntity->BaseEntity():NULL;
	m_vecStandoffGoalPosition = GOAL_POSITION_INVALID;
...
{
	CEntity *pLeader = GetPlayerLeader();
	...
	if ( GetEnemy() )
	{
		AI_EnemyInfo_t *pEnemyInfo = GetOuter()->GetEnemies()->Find( GetEnemy()->BaseEntity() );
		if ( pEnemyInfo &&
			     ...
			     gpGlobals->curtime  - pEnemyInfo->timeFirstSeen > m_params.flAbandonTimeLimit * 2 ) ) )
		{
			...
			if ( pSquad )
			{
				for ( CBaseEntity *pSquadMate = pSquad->GetFirstMember( &iter ); pSquadMate; pSquadMate = pSquad->GetNextMember( &iter ) )
				{
					CAI_NPC *npc = (CAI_NPC *)CEntity::Instance(pSquadMate);
					if ( npc->IsAlive() && npc != GetOuter() )
					{
						CAI_StandoffBehavior *pSquadmateStandoff;
						npc->GetBehavior( &pSquadmateStandoff );
...
int CAI_StandoffBehavior::SelectScheduleUpdateWeapon( void )
{
	...
	{
		// if hurt:
		int iPercent = enginerandom->RandomInt(0,99);
...
int CAI_StandoffBehavior::SelectScheduleCheckCover( void )
{
	...
	if ( GetOuter()->GetShotRegulator()->IsInRestInterval() )
	{
		...
		if ( enginerandom->RandomInt(0,99) < 80 )
			SetReuseCurrentCover();
		return SCHED_TAKE_COVER_FROM_ENEMY;
	}
...
int CAI_StandoffBehavior::SelectScheduleAttack( void )
{
	if ( GetPosture() == AIP_PEEKING || GetPosture() == AIP_STANDING )
	{
		...
		{
			if ( GetOuter()->GetActiveWeapon() && ( GetOuter()->GetActiveWeapon()->Weapon_CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK1 ) )
			{
				if ( !HasCondition( COND_ENEMY_OCCLUDED ) || enginerandom->RandomInt(0,99) < 50 )
...
Vector CAI_StandoffBehavior::GetStandoffGoalPosition() 
{
	...
	{
		CE_CAI_BattleLine *pBattleLine = NULL;
		for (;;)
		{
			pBattleLine = (CE_CAI_BattleLine *)g_helpfunc.FindEntityByClassname( pBattleLine, "ai_battle_line" );
...
bool CAI_StandoffBehavior::IsValidCover( const Vector &vecCoverLocation, const CBaseEntity *pHint )
{
...
bool CAI_StandoffBehavior::IsValidShootPosition( const Vector &vLocation, CAI_Node *pNode, CBaseEntity const *pHint )
{
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_FIND_COVER_FROM_ENEMY:
		{
			...
			if ( !m_params.fStayAtCover && (!m_TimeForceCoverHint.Expired() || m_RandomCoverChangeTimer.Expired()) )
			{
				...
				{
					if( enginerandom->RandomInt(1, 100) <= 50 )
					{
...
{
	CPlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin());
	return ( pPlayer && GetOuter()->IRelationType( pPlayer->BaseEntity() ) == D_LI );
}

//-------------------------------------

CEntity *CAI_StandoffBehavior::GetPlayerLeader()
{
	CPlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin());
	if ( pPlayer && GetOuter()->IRelationType( pPlayer->BaseEntity() ) == D_LI )
		return pPlayer;
	return NULL;
}
...
{
	CE_AI_Hint *pHintNode = GetHintNode();
	if ( pHintNode )
...
Activity CAI_StandoffBehavior::GetCoverActivity()
{
	...
	if ( pHintNode && pHintNode->HintType() == HINT_TACTICAL_COVER_LOW )
		return GetOuter()->GetCoverActivity( pHintNode->BaseEntity() );
	return ACT_INVALID;
}
...
void CAI_MappedActivityBehavior_Temporary::UpdateTranslateActivityMap()
{
	...
	CCombatWeapon *pWeapon = GetOuter()->GetActiveWeapon();
...
void CAI_MappedActivityBehavior_Temporary::OnChangeActiveWeapon( CBaseEntity *pOldWeapon, CBaseEntity *pNewWeapon )
{
	UpdateTranslateActivityMap();
}
