...
void CNPC_Combine::InputThrowGrenadeAtTarget( inputdata_t &inputdata )
{
	...
	if ( m_NPCState == NPC_STATE_SCRIPT && Get_m_hCine() )
		return;
	...
	m_hForcedGrenadeTarget = pEntity->BaseEntity();
...
void CNPC_Combine::DelaySquadAltFireAttack( float flDelay )
{
	...
	AISquadIter_t iter;
	CBaseEntity *cbase = GetSquad() ? GetSquad()->GetFirstMember( &iter ) : NULL;
	while ( cbase )
	{
		CAI_NPC *pSquadmate = (CAI_NPC *)CEntity::Instance(cbase); 
		...
		cbase = GetSquad()->GetNextMember( &iter );
	}
}
...
bool CNPC_Combine::ShouldMoveAndShoot()
{
	...
	if( IsCurSchedule( SCHED_COMBINE_HIDE_AND_RELOAD, false ) )
		m_flStopMoveShootTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.4f, 0.6f );
	...
	if( GetSquad() && IsCurSchedule( SCHED_COMBINE_TAKE_COVER1, false ) )
		m_flStopMoveShootTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.4f, 0.6f );
...
void CNPC_Combine::StartTaskChaseEnemyContinuously( const Task_t *pTask )
{
	...
	if ( IsUnreachable( pEnemy->BaseEntity() ) )
	{
...
void CNPC_Combine::StartTask( const Task_t *pTask )
{
	...
	case TASK_COMBINE_SIGNAL_BEST_SOUND:
		if( IsInSquad() && GetSquad()->NumMembers() > 1 )
		{
			...
			if( pPlayer && OccupyStrategySlot( SQUAD_SLOT_EXCLUSIVE_HANDSIGN ) && pPlayer->FInViewCone_Entity( BaseEntity() ) )
			{
				CSound *pSound;
				pSound = GetBestSound();
	...
	case TASK_ANNOUNCE_ATTACK:
		{
			...
			if ((int)pTask->flTaskData == 1)
			{
				...
				// -----------------------------------------------------------
				CCombatCharacter *pBCC = GetEnemyCombatCharacterPointer();

				if	(pBCC && pBCC->IsPlayer() && (!pBCC->FInViewCone_Entity ( BaseEntity() )) &&
	...
	case TASK_COMBINE_GET_PATH_TO_FORCED_GREN_LOS:
		{
			...
			CEntity *cent = CEntity::Instance(m_hForcedGrenadeTarget);
			Vector vecEnemy = cent->GetAbsOrigin();
			Vector vecEnemyEye = vecEnemy + cent->GetViewOffset();
	...
	case TASK_COMBINE_IGNORE_ATTACKS:
		// must be in a squad
		if (GetSquad() && GetSquad()->NumMembers() > 2)
		{
	...
	case TASK_COMBINE_DEFER_SQUAD_GRENADES:
		{
			if ( GetSquad() )
			{
				...
				CBaseEntity *cbase = GetSquad()->GetFirstMember( &iter );
				while ( cbase )
				{
					CAI_NPC *pSquadmate = (CAI_NPC*)CEntity::Instance(cbase);
					CNPC_Combine *pCombine = dynamic_cast<CNPC_Combine*>(pSquadmate);
	...
	case TASK_ANNOUNCE_ATTACK:
		{
			...
			CCombatCharacter* pBCC = GetEnemyCombatCharacterPointer();
			if	(!pBCC || pBCC->FInViewCone_Entity( BaseEntity() ))
			{
				TaskComplete();
			}
	...
	case TASK_COMBINE_GET_PATH_TO_FORCED_GREN_LOS:
		{
			...
			if ( GetTaskInterrupt() > 0 )
			{
				...
				Vector vecEnemy = CEntity::Instance(m_hForcedGrenadeTarget)->GetAbsOrigin();
	...
	case TASK_RANGE_ATTACK1:
		{
			...
			if (!FInAimCone_Vector( vecEnemyLKP ))
			{
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_Combine::FVisible_Entity( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
	...
	if( GetActivity() == ACT_RANGE_ATTACK2 )
	{
		if( m_iLastAnimEventHandled != COMBINE_AE_GREN_TOSS )
		{
			...
			if ( pItem )
			{
				...
				{
					Vector			vel;
					vel.x = enginerandom->RandomFloat( -100.0f, 100.0f );
					vel.y = enginerandom->RandomFloat( -100.0f, 100.0f );
					vel.z = enginerandom->RandomFloat( 800.0f, 1200.0f );
...
{
	if (enginerandom->RandomInt(0,5) > 1)
		return;
	...
	if (!pBCC->FInViewCone_Entity ( BaseEntity() ))
		return;

	// Make sure player can see me
	if ( FVisible_Entity( pBCC->BaseEntity() ) )
	{
...
void CNPC_Combine::AnnounceEnemyType( CEntity *pEnemy )
{
...
void CNPC_Combine::AnnounceEnemyKill( CEntity *pEnemy )
{
	if (!pEnemy )
		return;
...
int CNPC_Combine::SelectCombatSchedule()
{
	...
	if ( HasCondition( COND_NEW_ENEMY ) )
	{
		...
		float flTimeSinceFirstSeen = gpGlobals->curtime - GetEnemies()->FirstTimeSeen( GetEnemy_CBase() );
		...
		if ( GetSquad() && pEnemy )
		{
			...
			if ( GetSquad()->IsLeader( this ) || ( GetSquad()->GetLeader() && GetSquad()->GetLeader()->GetEnemy() != pEnemy ) )
			{
			...
			{
				if ( GetSquad()->GetLeader() && FOkToMakeSound( SENTENCE_PRIORITY_MEDIUM ) )
				{
				...
				{
					if( enginerandom->RandomInt(0, 100) < 60 )
					{
	...
	if ( HasCondition( COND_LIGHT_DAMAGE ) )
	{
		if ( GetEnemy() != NULL )
		{
			...
			if( !IsCrouching() )
			{
				if( GetEnemy() && enginerandom->RandomFloat( 0, 100 ) < 50 && CouldShootIfCrouching( GetEnemy() ) )
				{
					Crouch();
				}
				else
	...
	if ( IRelationType( GetEnemy_CBase() ) == D_FR )
	{
		if (HasCondition( COND_SEE_ENEMY )	|| 
		...
		AI_EnemyInfo_t *pMemory = GetEnemies()->Find( GetEnemy_CBase() );
		if ( (pMemory && pMemory->bUnforgettable) || (GetEnemyLastTimeSeen() > (gpGlobals->curtime - 5.0)) )
		{
			...
			if ( FInAimCone_Vector( GetEnemy()->EyePosition() ) )
...
int CNPC_Combine::SelectSchedule( void )
{
	...
	if ( m_hForcedGrenadeTarget )
	{
		...
		{
			Vector vecTarget = CEntity::Instance(m_hForcedGrenadeTarget)->WorldSpaceCenter();

			if ( IsElite() )
			{
				if ( FVisible_Entity( m_hForcedGrenadeTarget ) )
				{
			...
			if ( HasCondition(COND_HEAR_DANGER) )
			{
				...
				if ( pSound)
				{
					if (pSound->m_iType & SOUND_DANGER)
					{
						...
						CEntity *pSoundOwner = CEntity::Instance(pSound->m_hOwner);
						if ( pSoundOwner )
						{
							CE_Grenade *pGrenade = dynamic_cast<CE_Grenade *>(pSoundOwner);
							if ( pGrenade && pGrenade->GetThrower() )
							{
								CCombatCharacter *thrower = pGrenade->GetThrower();
								if ( IRelationType( (thrower)?thrower->BaseEntity():NULL ) != D_LI )
								{
						...
						if( !GetEnemy() && pSound->IsSoundType(SOUND_CONTEXT_DANGER_APPROACH) && pSound->m_hOwner && !FInViewCone_Vector(pSound->GetSoundReactOrigin()) )
						{
	...
	case NPC_STATE_ALERT:
		{
			if( HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE) )
			{
				...
				if( pDanger && FInViewCone_Vector(pDanger->vLastKnownLocation) && !BaseClass::FVisible_Vector(pDanger->vLastKnownLocation) )
				{
			...
			if( HasCondition( COND_HEAR_COMBAT ) )
			{
				...
				{
					if( GetSquad() && GetSquad()->GetSquadMemberNearestTo( pSound->GetSoundReactOrigin() ) == this && OccupyStrategySlot( SQUAD_SLOT_INVESTIGATE_SOUND ) )
					{
...
int CNPC_Combine::SelectScheduleAttack()
{
	...
	if ( GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE && FClassnameIs(GetEnemy(), "npc_turret_floor") )
	{
		...
		float flTimeAtFirstHand = GetEnemies()->TimeAtFirstHand(GetEnemy_CBase());
		...
		if ( !GetEnemy()->MyNPCPointer()->FInViewCone_Entity( BaseEntity() ) && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) )
	...
	if ( ShouldChargePlayer() )
	{
		...
		if ( HasCondition(COND_SEE_ENEMY) && !IsUnreachable( GetEnemy_CBase() ) )
		{
	...
	if ( GetEnemy() && !HasCondition(COND_SEE_ENEMY) )
	{
		...
		flTime = gpGlobals->curtime - GetEnemies()->LastTimeSeen( GetEnemy_CBase() );
		flDist = ( GetEnemy()->GetAbsOrigin() - GetEnemies()->LastSeenPosition( GetEnemy_CBase() ) ).Length();
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( GetSquad() )
			{
				...
				{
					if ( ShouldChargePlayer() && !IsUnreachable( GetEnemy_CBase() ) )
						return SCHED_COMBINE_CHARGE_PLAYER;
			...
			{
				// Have to explicitly check innate range attack condition as may have weapon with range attack 2
				if ( enginerandom->RandomInt(0,1) && HasCondition(COND_CAN_RANGE_ATTACK2) )
				{
	...
	case SCHED_HIDE_AND_RELOAD:
		{
			...
			// Stand();
			// DesireStand();
			if( CanGrenadeEnemy() && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) && enginerandom->RandomInt( 0, 100 ) < 20 )
			{
	...
	case SCHED_RANGE_ATTACK2:
		{
			...
			if (GetActiveWeapon() && GetActiveWeapon()->Weapon_CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK2)
			{
				return SCHED_RANGE_ATTACK2;
			}
...
void CNPC_Combine::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if (pEvent->type & AE_TYPE_NEWEVENTSYSTEM)
	{
		...
		else if ( pEvent->event == COMBINE_AE_ALTFIRE )
		{
			if( IsElite() )
			{
				...
				fakeEvent.pSource = BaseEntity();
				fakeEvent.event = EVENT_WEAPON_AR2_ALTFIRE;
				GetActiveWeapon()->Operator_HandleAnimEvent( &fakeEvent, BaseEntity() );
	...
	else
	{
		...
		case COMBINE_AE_GREN_TOSS:
			{
				Vector vecSpin;
				vecSpin.x = enginerandom->RandomFloat( -1000.0, 1000.0 );
		...
		case COMBINE_AE_GREN_LAUNCH:
			{
				...
				*/
				...
				if ( g_helpfunc.GameRules_IsSkillLevel(SKILL_HARD) )
					m_flNextGrenadeCheck = gpGlobals->curtime + enginerandom->RandomFloat( 2, 5 );// wait a random amount of time before shooting again
				else
		...
		case COMBINE_AE_KICK:
			{
				...
				CBaseEntity *pHurt = CheckTraceHullAttack_Float( 70, -Vector(16,16,18), Vector(16,16,18), 0, DMG_CLUB );
				CCombatCharacter* pBCC = ToBaseCombatCharacter( CEntity::Instance(pHurt) );
				if (pBCC)
				{
					Vector forward, up;
					AngleVectors( GetLocalAngles(), &forward, NULL, &up );

					if ( !pBCC->DispatchInteraction( g_interactionCombineBash, NULL, BaseEntity() ) )
					{
						if ( pBCC->IsPlayer() )
						{
							pBCC->ViewPunch( QAngle(-12,-7,0) );
							pBCC->ApplyAbsVelocityImpulse( forward * 100 + up * 50 );
						}

						CTakeDamageInfo info( BaseEntity(), BaseEntity(), m_nKickDamage, DMG_CLUB );
...
void CNPC_Combine::LostEnemySound( void)
{
	...
	{
		m_flNextLostSoundTime = gpGlobals->curtime + enginerandom->RandomFloat(5.0,15.0);
	}
}
...
{
	if (g_fCombineQuestion || enginerandom->RandomInt(0,1))
	{
		if (!g_fCombineQuestion)
		{
			// ask question or make statement
			switch (enginerandom->RandomInt(0,2))
			{
			case 0: // check in
...
bool CNPC_Combine::CanThrowGrenade( const Vector &vecTarget )
{
	...
	if ( GetSquad() )
	{
		if (GetSquad()->SquadMemberInRange( vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST ))
		{
			...
			g_helpfunc.CSoundEnt_InsertSound( SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY, vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST, 0.1 );
			return false;
		}
	}
...
bool CNPC_Combine::CheckCanThrowGrenade( const Vector &vecTarget )
{
	...
	if( FInViewCone_Vector( vecTarget ) && CBaseEntity_FVisible( vecTarget ) )
	{
	...
	else
	{
		...
		trace_t tr;
		UTIL_TraceLine( EyePosition(), EyePosition() + Vector( 0, 0, 64 ), MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
		if( tr.fraction != 1.0 )
		{
			return false;
		}
...
bool CNPC_Combine::CanAltFireEnemy( bool bUseFreeKnowledge )
{
	...
	{
		vecTarget = GetEnemies()->LastKnownPosition( pEnemy->BaseEntity() ) + (pEnemy->GetViewOffset()*0.75);// approximates the chest
	}
	else
	{
		vecTarget = GetEnemies()->LastSeenPosition( pEnemy->BaseEntity() ) + (pEnemy->GetViewOffset()*0.75);// approximates the chest
	}
	...
	UTIL_TraceHull( vShootPosition, vecTarget, mins, maxs, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
bool CNPC_Combine::CanGrenadeEnemy( bool bUseFreeKnowledge )
{
	...
	if( pEnemy )
	{
		...
		if( bUseFreeKnowledge )
		{
			...
			return CanThrowGrenade( GetEnemies()->LastKnownPosition( pEnemy->BaseEntity() ) );
		}
		else
		{
			// hafta throw to where we last saw them.
			return CanThrowGrenade( GetEnemies()->LastSeenPosition( pEnemy->BaseEntity() ) );
		}
	}
...
int CNPC_Combine::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	UTIL_TraceLine(vecSrc, vecEnd, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);
