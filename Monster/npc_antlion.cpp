...
void CNPC_Antlion::Spawn( void )
{
	...
	m_nSkin = enginerandom->RandomInt( 0, ANTLION_SKIN_COUNT-1 );
}
...
//-----------------------------------------------------------------------------
inline CEntity *CNPC_Antlion::EntityToWatch( void )
{
	return ( m_hFollowTarget ) ? m_hFollowTarget : GetEnemy();
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::FInViewCone_Entity( CBaseEntity *pEntity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::FInViewCone_Vector( const Vector &vecSpot )
{
	m_flFieldOfView = ( GetEnemy() != NULL ) ? ANTLION_VIEW_FIELD_NARROW : VIEW_FIELD_WIDE;

	return BaseClass::FInViewCone_Vector( vecSpot );
}
...
void CNPC_Antlion::Event_Killed( const CTakeDamageInfo &info )
{
	...
	{
		g_helpfunc.CSoundEnt_InsertSound(SOUND_PHYSICS_DANGER, GetAbsOrigin(), 256, 0.5f, BaseEntity() );
	}

	BaseClass::Event_Killed( info );

	CEntity *pAttacker = CEntity::Instance(info.GetInflictor());

	if ( pAttacker && pAttacker->GetServerVehicle() && ShouldGib( info ) == true )
	{
		trace_t tr;
		UTIL_TraceLine( GetAbsOrigin() + Vector( 0, 0, 64 ), pAttacker->GetAbsOrigin(), MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CNPC_Antlion::MeleeAttack( float distance, float damage, QAngle& viewPunch, Vector& shove )
{
	...
	if ( ( GetEnemy() != NULL ) && ( GetEnemy()->Classify() == CLASS_BULLSEYE ) )
	{
		...
		CTakeDamageInfo info( BaseEntity(), BaseEntity(), damage, DMG_SLASH );
		CalculateMeleeDamageForce( &info, vecForceDir, GetEnemy()->GetAbsOrigin() );
		GetEnemy()->TakeDamage( info );
		return;
	}

	CBaseEntity *cbase = CheckTraceHullAttack_Float( distance, -Vector(16,16,32), Vector(16,16,32), (int)damage, DMG_SLASH, 5.0f );
	CEntity *pHurt = CEntity::Instance(cbase);
	...
	if ( pHurt )
	{
		...
		{
			int health = pHurt->m_iHealth;
			CTakeDamageInfo	dmgInfo( BaseEntity(), BaseEntity(), health+25, DMG_SLASH );
...
bool CNPC_Antlion::FindChasePosition( const Vector &targetPos, Vector &result )
{
	...
	{
		result	= targetPos;
		result += -runDir * enginerandom->RandomInt( 64, 128 );
		result += vRight * enginerandom->RandomInt( -128, 128 );
...
bool CNPC_Antlion::GetGroundPosition( const Vector &testPos, Vector &result )
{
	...
	trace_t	tr;
	UTIL_TraceHull( testPos, testPos + Vector( 0, 0, 64 ), NAI_Hull::Mins( GetHullType() ), NAI_Hull::Maxs( GetHullType() ), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	UTIL_TraceHull( tr.endpos, tr.endpos - Vector( 0, 0, 128 ), NAI_Hull::Mins( GetHullType() ), NAI_Hull::Maxs( GetHullType() ), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
bool CNPC_Antlion::GetPathToSoundFleePoint( int soundType )
{
	...
	CE_AI_Hint *pHint = CAI_HintManager::FindHint( WorldSpaceCenter(), hintCriteria );
	...
	float flFleeYaw = VecToYaw( vecFleeDir ) + enginerandom->RandomInt( -20, 20 );

	vecFleeDir = UTIL_YawToVector( flFleeYaw );

	// Move us to the outer radius of the noise (with some randomness)
	vecFleeGoal = vecSoundPos + vecFleeDir * ( pSound->Volume() + enginerandom->RandomInt( 32, 64 ) );
...
{
	float flLastSeenTime = GetEnemies()->LastTimeSeen( GetEnemy_CBase() );
...
bool CNPC_Antlion::GetSpitVector( const Vector &vecStartPos, const Vector &vecTarget, Vector *vecOut )
{
	...
	Vector vecToss = VecCheckThrowTolerance( BaseEntity(), vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat(), (10.0f*12.0f) );

	// If this failed then try a little faster (flattens the arc)
	if ( vecToss == vec3_origin )
	{
		vecToss = VecCheckThrowTolerance( BaseEntity(), vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat() * 1.5f, (10.0f*12.0f) );
		if ( vecToss == vec3_origin )
			return false;
	}
...
void CNPC_Antlion::DelaySquadAttack( float flDuration )
{
	if ( GetSquad() )
	{
		...
		float flAdjDuration = flDuration - enginerandom->RandomFloat( 0.0f, (flDuration*0.5f) );
...
void CNPC_Antlion::HandleAnimEvent( animevent_t *pEvent )
{
	...
	Vector shove;
		...
		if ( pEvent->event == AE_ANTLION_WORKER_SPIT )
		{
			if ( GetEnemy() )
			{
				...
				g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, vTarget, (15*12), flTime, BaseEntity());

				// Don't fire again until this volley would have hit the ground (with some lag behind it)
				SetNextAttack( gpGlobals->curtime + flTime + enginerandom->RandomFloat( 0.5f, 2.0f ) );

				// Tell any squadmates not to fire for some portion of the time this volley will be in the air (except on hard)
				if ( g_helpfunc.GameRules_IsSkillLevel( SKILL_HARD ) == false )
				...
				for ( int i = 0; i < 6; i++ )
				{
					...
					DispatchSpawn( pGrenade->BaseEntity() );
					pGrenade->SetThrower( BaseEntity() );
					pGrenade->SetOwnerEntity( BaseEntity() );
					...
					{
						pGrenade->SetSpitSize( enginerandom->RandomInt( SPIT_SMALL, SPIT_MEDIUM ) );
					...
					pGrenade->SetLocalAngularVelocity(
						QAngle( enginerandom->RandomFloat( -250, -500 ),
								enginerandom->RandomFloat( -250, -500 ),
								enginerandom->RandomFloat( -250, -500 ) ) );
				}
	...
	{
		g_helpfunc.CSoundEnt_InsertSound( SOUND_PHYSICS_DANGER, GetAbsOrigin(), sk_antlion_worker_burst_radius.GetInt(), 0.5f, BaseEntity() );
		return;
	}

	if ( pEvent->event == AE_ANTLION_WORKER_EXPLODE )
	{
		CTakeDamageInfo info( BaseEntity(), BaseEntity(), sk_antlion_worker_burst_damage.GetFloat(), DMG_BLAST_SURFACE | ( ANTLION_WORKER_BURST_IS_POISONOUS() ? DMG_POISON : DMG_ACID ) );
		Event_Gibbed( info );
		return;
	}
...
bool CNPC_Antlion::IsUnusableNode(int iNodeID, CBaseEntity *pHint)
{
...
bool NPC_Antlion_IsAntlion( CEntity *pEntity )
{
...
{
public:
	CTraceFilterAntlion( const CEntity *pEntity ) { m_pIgnore = pEntity; }
...
void CNPC_Antlion::StartTask( const Task_t *pTask )
{
	...
	case TASK_ANTLION_DROWN:
	{
		...
		m_flTimeDrownSplash = gpGlobals->curtime + enginerandom->RandomFloat( 0, 0.5 );
	...
	case TASK_ANTLION_DISMOUNT_NPC:
		{
			CEntity *pGroundEnt = GetGroundEntity();
			...
			if( pGroundEnt != NULL )
			{
				...
				{
					m_bDontExplode = true;
					OnTakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), *(m_iHealth)+1, DMG_GENERIC ) );
					return;
				}
	...
	case TASK_ANTLION_BURROW_WAIT:
		...
		if ( pTask->flTaskData == 1.0f )
		{
			...
			m_flBurrowTime = gpGlobals->curtime + enginerandom->RandomFloat( 1, 6 );
		}
...
void CNPC_Antlion::RunTask( const Task_t *pTask )
{
	...
	case TASK_ANTLION_DROWN:
	{
		if ( gpGlobals->curtime > m_flTimeDrownSplash )
		{
			...
			data.m_flScale = enginerandom->RandomFloat( 12.0, 16.0 );

			g_helpfunc.DispatchEffect( "watersplash", data );
			
			m_flTimeDrownSplash = gpGlobals->curtime + enginerandom->RandomFloat( 0.5, 2.5 );
		}
	
		if ( gpGlobals->curtime > m_flTimeDrown )
		{
			m_bDontExplode = true;
			OnTakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), *(m_iHealth)+1, DMG_DROWN ) );
			TaskComplete();
		}
		break;
	}
	...
	case TASK_ANTLION_DISMOUNT_NPC:
		...
		if ( GetFlags() & FL_ONGROUND )
		{
			...
			if ( ( pGroundEnt != NULL ) && ( ( pGroundEnt->MyNPCPointer() != NULL ) || pGroundEnt->GetSolidFlags() & FSOLID_NOT_STANDABLE ) )
			{
				...
				vecRandom[2] = enginerandom->RandomFloat( 100.0f, 200.0f );
	...
	case TASK_ANTLION_WAIT_FOR_TRIGGER:
		...
		if ( ( m_flIdleDelay > gpGlobals->curtime ) || GetEntityName() != NULL )
			return;
	...
	case TASK_ANTLION_CHECK_FOR_UNBORROW:
		...
		m_flBurrowTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.5f, 1.0f );
		m_iUnBurrowAttempts++;
...
bool CNPC_Antlion::IsJumpLegal( const Vector &startPos, const Vector &apex, const Vector &endPos ) const
{
	...
	{
		trace_t	tr;
		UTIL_TraceHull( endPos, endPos, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
		
		CEntity *cent = CEntity::Instance(tr.m_pEnt);
		if ( cent )
		{
			CAI_NPC *pBlocker = cent->MyNPCPointer();
...
//	NDebugOverlay::Line( vOrigin, vOrigin - Vector( 0, 0, flHeight * 0.5  ), 255, 0, 0, true, 5 );
	UTIL_TraceLine( vOrigin, vOrigin - Vector( 0, 0, flHeight * 0.5  ), MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), &tr );
...
int CNPC_Antlion::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( m_FollowBehavior.GetNumFailedFollowAttempts() >= 2 )
	{
		if( IsFirmlyOnGround() == false )
		{
			...
			{
				vecJumpDir.x = enginerandom->RandomInt( -1, 1 ); 
				vecJumpDir.y = enginerandom->RandomInt( -1, 1 );
			}
...
bool CNPC_Antlion::ShouldJump( void )
{
	...
	{
		m_flJumpTime = gpGlobals->curtime + enginerandom->RandomFloat( 1.0f, 2.0f );		
		return false;
	}
...
void CNPC_Antlion::ZapThink( void )
{
	...
	data.m_flScale = enginerandom->RandomFloat( 0.25f, 1.0f );

	g_helpfunc.DispatchEffect( "TeslaHitboxes", data );
...
int CNPC_Antlion::SelectSchedule( void )
{
	...
	if ( HasCondition( COND_HEAR_BUGBAIT ) && ( m_bIgnoreBugbait == false ) )
	{
		...
		{		
			...
			while ( ( pTarget = g_helpfunc.FindEntityInSphere( pTarget, soundOrg, 512 ) ) != NULL )
			{
				if(pTarget == NULL)
					continue;

				CAI_NPC *pNPC = pTarget->MyNPCPointer();

				if ( pNPC == NULL )
					continue;

				if ( pNPC->CanBeAnEnemyOf( pTarget->BaseEntity() ) == false )
					continue;

				//Check to see if the default relationship is hatred, and if so intensify that
				if ( ( IRelationType( pTarget->BaseEntity() ) == D_HT ) && ( pNPC->IsPlayer() == false ) )
				{
					AddEntityRelationship( pTarget->BaseEntity(), D_HT, 99 );
					
					//Try to spread out the enemy distribution
					if ( ( pNewEnemy == NULL ) || ( enginerandom->RandomInt( 0, 1 ) ) )
					{
						pNewEnemy = pTarget->BaseEntity();
						continue;
					}
				}
			}
	...
	case NPC_STATE_COMBAT:
		{
			...
			if ( hl2_episodic->GetBool() && IsWorker() )
			{
				...
				{
					SetNextAttack( gpGlobals->curtime + enginerandom->RandomFloat( 2.0f, 4.0f ) );
				...
				if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
				{
					...
					{
						EmitSound( "NPC_Antlion.PoisonBurstScream" );
						SetNextAttack( gpGlobals->curtime + enginerandom->RandomFloat( 0.5f, 2.5f ) );
						if ( GetEnemy() )
						{
	...
	default:
		{
			...
			if ( GetEnemy() == NULL && ( HasCondition( COND_LIGHT_DAMAGE ) || HasCondition( COND_HEAVY_DAMAGE ) ) )
			{
				...
				if ( FInViewCone_Vector( vecEnemyLKP ) )
				{
...
void CNPC_Antlion::Ignite ( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
	...
	CTakeDamageInfo	dmgInfo( BaseEntity(), BaseEntity(), flDamage, DMG_GENERIC );
...
int CNPC_Antlion::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( hl2_episodic->GetBool() )
	{
		...
		{
			CEntity *cent = CEntity::Instance(newInfo.GetInflictor());
			if( cent && cent->VPhysicsGetObject() )
			{
				float flMass = cent->VPhysicsGetObject()->GetMass();
...
void CNPC_Antlion::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( IsFlipped() )
	{
		...
		{
			PainSound( newInfo );
			Vector vecForce = ( vecShoveDir * enginerandom->RandomInt( 500, 1000 ) ) + Vector(0.0f, 0.0f, 64.0f);
		...
		if ( !IsRunningDynamicInteraction() )
 		{
			...
			else
			{
				...
				if ( GetFlags() & FL_ONGROUND )
				{
					...
					//Get tossed!
					ApplyAbsVelocityImpulse( ( vecShoveDir * enginerandom->RandomInt( 500, 1000 ) ) + Vector(0.0f, 0.0f, 64.0f) );
					SetGroundEntity( NULL );
				}
			}
		}
...
bool CNPC_Antlion::ShouldPlayIdleSound( void )
{
	...
	//Randomize it a bit
	if ( enginerandom->RandomInt( 0, 20 ) )
		return false;
...
bool CNPC_Antlion::FCanCheckAttacks( void )
{
	if ( IsWorker() )
	{
		...
		if ( SeenEnemyWithinTime( 3.0f ) && ConditionInterruptsCurSchedule( COND_CAN_RANGE_ATTACK1 ) )
			return FInViewCone_Entity( GetEnemy_CBase() );
	}
...
int CNPC_Antlion::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	trace_t	tr;
	UTIL_TraceHull( WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(), -Vector(8,8,8), Vector(8,8,8), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );

	// If the hit entity isn't our target and we don't hate it, don't hit it
	if ( tr.m_pEnt != GetEnemy_CBase() && tr.fraction < 1.0f && IRelationType( tr.m_pEnt ) != D_HT )
		return 0;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::HandleInteraction( int interactionType, void *data, CBaseEntity *sender )
{
...
void CNPC_Antlion::StartJump( void )
{
	...
	m_flJumpTime = gpGlobals->curtime + enginerandom->RandomInt( 2, 6 );
}
...
//-----------------------------------------------------------------------------
bool CNPC_Antlion::FValidateHintType( CBaseEntity *pHint )
{
	...
	case ANTLION_BURROW_OUT:
		{			
			...
			Vector vHintPos;
			hint->GetPosition(this,&vHintPos);
...
// traceline methods
//-----------------------------------------------------------------------------
class CTraceFilterSimpleNPCExclude : public CE_CTraceFilterSimple
{
public:
	DECLARE_CLASS( CTraceFilterSimpleNPCExclude, CE_CTraceFilterSimple );
	...
	bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if ( GetPassEntity() )
		{
			CEntity *pEnt = CEntity::Instance(GetPassEntity()->GetRefEHandle());

			if ( pEnt->IsNPC() )
			{
				if ( NPC_CheckBrushExclude( pEnt, CEntity::Instance(pTestEntity) ) == true )
					return false;
			}
		}
...
bool CNPC_Antlion::ValidBurrowPoint( const Vector &point )
{
	...
	CTraceFilterSimpleNPCExclude filter( BaseEntity(), COLLISION_GROUP_NONE );
	UTIL_TraceHull( point, point+Vector(0,0,1), GetHullMins(), GetHullMaxs(), 
...
bool CNPC_Antlion::FindBurrow( const Vector &origin, float distance, int type, bool excludeNear )
{
	...
	if ( type == ANTLION_BURROW_IN )
	{
		...
		CE_AI_Hint *pHint = CAI_HintManager::FindHint( this, hintCriteria );
...
void CNPC_Antlion::BurrowUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( ( cent_pCaller != NULL ) && ( cent_pCaller->IsPlayer() ) )
	{
		SetEnemy( pActivator );
	}
...
bool CNPC_Antlion::CheckLanding( void )
{
	...
	// Look below
	UTIL_TraceHull( GetAbsOrigin(), testPos, NAI_Hull::Mins( GetHullType() ), NAI_Hull::Maxs( GetHullType() ), MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	if ( ( tr.fraction != 1.0f ) || ( GetFlags() & FL_ONGROUND ) )
	{
		...
		if ( GetSequence() != sequence )
		{
			...
			{
				CPlayer *pPlayer = ToBasePlayer( GetEnemy() );
...
{
	trace_t	tr;
	UTIL_TraceLine( GetAbsOrigin()+Vector(0,0,1), GetAbsOrigin()-Vector(0,0,64), MASK_SOLID_BRUSHONLY | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	if ( tr.fraction < 1.0f )
	{
			 ...
			 ( pdata->game.material == CHAR_TEX_SAND ) ) 
		{
			...
			{			
				UTIL_CreateDust(tr.endpos + Vector(0, 0, 24), GetAbsAngles(), 6.0f, 10.0f);
...
bool CNPC_Antlion::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( IsAllied() && cent_pEnemy->IsPlayer() )
		return false;

	if ( cent_pEnemy->IsWorld() )
		return false;
	...
	if ( IsCurSchedule( GetGlobalScheduleId( SCHED_ANTLION_CHASE_BUGBAIT ) ) && ( GetNavigator() != NULL ) )
	{
		...
		if ( ( GetNavigator()->IsGoalActive() ) && ( GetNavigator()->GetGoalPos() - cent_pEnemy->GetAbsOrigin() ).Length() > 512 )
			return false;
	}
	...
	{
		float enemyDist = ( cent_pEnemy->GetAbsOrigin() - GetAbsOrigin() ).LengthSqr();
...
{
	if ( ( enginerandom->RandomInt( 0, 1 ) ) && ( info.GetDamage() > 3 ) )
		return true;
...
bool CNPC_Antlion::ShouldResumeFollow( void )
{
	...
	if ( GetEnemy() != NULL )
	{
		m_flSuppressFollowTime = gpGlobals->curtime + enginerandom->RandomInt( 5, 10 );
		return false;
	}
...
void CNPC_Antlion::InputFightToPosition( inputdata_t &inputdata )
{
	...
	CEntity *pEntity = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.String(), NULL, inputdata.pActivator, inputdata.pCaller );
...
bool CNPC_Antlion::CorpseGib( const CTakeDamageInfo &info )
{
	...
	else
	{
		...
		//CE_TODO
		//g_helpfunc.DispatchParticleEffect( "AntlionGib", vecOrigin, QAngle( 0, 0, 0 ) );
	}
	...
	PropBreakableCreateAll( GetModelIndex(), NULL, params, BaseEntity(), -1, true, true );
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_Antlion::Touch( CEntity *pOther )
{
	//See if the touching entity is a vehicle
	CPlayer *pPlayer = ToBasePlayer( pOther );
	...
	if ( pPlayer && pPlayer->IsInAVehicle() )
	{
		...
		CEntity *pVehicleEnt = CEntity::Instance(pVehicle->GetVehicleEnt());

		if ( pVehicleEnt == pOther )
		{
			CE_CPropVehicleDriveable *pDrivableVehicle = dynamic_cast<CE_CPropVehicleDriveable *>( pVehicleEnt );
			...
			if ( pDrivableVehicle != NULL )
			{
				...
				if ( ( ( pDrivableVehicle->m_nRPM > 75 ) && DotProduct( vecShoveDir, vecTargetDir ) <= 0 ) || bBurrowingOut == true )
				{
					if ( IsFlipped() || bBurrowingOut == true )
					{
						...
						if ( enginerandom->RandomInt( 0, 10 ) > 4 )
							 flDamage += 25;
									
						CTakeDamageInfo	dmgInfo( pVehicleEnt->BaseEntity(), pPlayer->BaseEntity(), flDamage, DMG_VEHICLE );
					...
					{
						// We're being shoved
						CTakeDamageInfo	dmgInfo( pVehicleEnt->BaseEntity(), pPlayer->BaseEntity(), 0, DMG_VEHICLE );
						PainSound( dmgInfo );
	...
	if ( GetActivity() == ACT_GLIDE && IsValidEnemy( pOther->BaseEntity() ) && !m_bHasDoneAirAttack )
	{
		CTakeDamageInfo	dmgInfo( BaseEntity(), BaseEntity(), sk_antlion_air_attack_dmg.GetInt(), DMG_SLASH );
	...
	if ( pOther->IsPlayer() )
	{
		...
		if ( IsValidEnemy( pOther->BaseEntity() ) )
			return;
	...
	if ( pOther->IsNPC() )
	{
		if ( pOther->Classify() == CLASS_COMBINE_GUNSHIP )
		{
			...
			CTakeDamageInfo	dmgInfo( pOther->BaseEntity(), pOther->BaseEntity(), flDamage, DMG_GENERIC );
...
bool CNPC_Antlion::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
	if ( hl2_episodic->GetBool() )
	{
		...
		{
			AddFacingTarget_E_V_F_F_F( GetEnemy_CBase(), GetEnemy()->WorldSpaceCenter(), 1.0f, 0.2f );
	...
	if ( IsCurSchedule( SCHED_ANTLION_FLEE_THUMPER ) )
	{
		...
		if ( pSound )
		{
			AddFacingTarget_V_F_F_F( pSound->GetSoundOrigin(), 1.0, 0.5f );
		}
	}
...
void CNPC_Antlion::SetMoveState( AntlionMoveState_e state )
{
	...
	case ANTLION_MOVE_FOLLOW:
		...
		m_FollowBehavior.SetFollowTarget( m_hFollowTarget.Get() );
	...
	case ANTLION_MOVE_FIGHT_TO_GOAL:
		...
		m_flSuppressFollowTime = gpGlobals->curtime + enginerandom->RandomInt( 10, 15 );
		break;
...
{ 
	...
	if ( m_nBodyBone == -1 )
	{
		...
		m_nBodyBone = LookupBone( "Antlion.Body_Bone" );
	}
...
{
	CEntity *pJumpTarget = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, inputdata.value.String(), BaseEntity(), inputdata.pActivator, inputdata.pCaller );
	if ( pJumpTarget == NULL )
	{
...
void CNPC_Antlion::DoPoisonBurst()
{
	if ( GetWaterLevel() < 2 )
	{
		...
		RadiusDamage( info, GetAbsOrigin(), sk_antlion_worker_burst_radius.GetFloat(), CLASS_NONE, BaseEntity() );

		DispatchParticleEffect( "blood_impact_antlion_01", WorldSpaceCenter(), GetAbsAngles() );
	}
	else
	{
		CEffectData	data;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool IsAntlionWorker( CEntity *pEntity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool IsAntlion( CEntity *pEntity )
{
	// Must at least be valid and an antlion
	return ( pEntity != NULL && 
			 pEntity->Classify() == CLASS_ANTLION && 
