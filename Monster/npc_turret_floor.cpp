...
void CNPC_FloorTurret::Activate( void )
{
	...
	if ( !m_pMotionController )
	{
		...
		CTurretTipController *control = CTurretTipController::CreateTipController( this );

		// Enable the controller
		if ( control != NULL )
		{
			control->Enable();
			m_pMotionController.Set(control->BaseEntity());
		}
	}
}
...
void CNPC_FloorTurret::Retire( void )
{
	...
	if ( GetActivity() != ACT_FLOOR_TURRET_CLOSE )
	{
		...
		if ( UpdateFacing() == false )
		{
			...
			m_OnRetire.FireOutput( (CBaseEntity *)NULL, BaseEntity() );
		}
	}
...
void CNPC_FloorTurret::Deploy( void )
{
	...
	if ( GetActivity() != ACT_FLOOR_TURRET_OPEN )
	{
		...
		m_OnDeploy.FireOutput( (CBaseEntity *)NULL, BaseEntity() );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_FloorTurret::HandleInteraction(int interactionType, void *data, CBaseEntity *sourceEnt)
{
	//CE_TODO
	/*if ( interactionType == g_interactionCombineBash )
	{
		...
		{
			m_hLastNPCToKickMe.Set(sourceEnt);
			m_flKnockOverFailedTime = gpGlobals->curtime + 3.0;
		}

		CCombatCharacter *cent_sourceEnt = (CCombatCharacter *)CEntity::Instance(sourceEnt);
		// Get knocked away
		Vector forward, up;
		AngleVectors( cent_sourceEnt->GetLocalAngles(), &forward, NULL, &up );
...
void CNPC_FloorTurret::ActiveThink( void )
{
	...
	bool bEnemyInFOV = FInViewCone_Entity( GetEnemy_CBase() );
	bool bEnemyVisible = FInViewCone_Entity( GetEnemy_CBase() ) && GetEnemy()->IsAlive();	
	...
	if ( bEnemyVisible )
	{
		...
		UTIL_TraceLine( vecCenter, vecMid, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction != 1.0 )
		{
			bEnemyVisible = false;
		}
...
void CNPC_FloorTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy, bool bStrict )
{
	...
	else
	{
		...
		info.m_iShots = 1;
		info.m_pAttacker = BaseEntity();
		info.m_vecSpread = GetAttackSpread( NULL, GetEnemy_CBase() );
		info.m_flDistance = MAX_COORD_RANGE;
		info.m_iAmmoType = m_iAmmoType;
	}
...
bool CNPC_FloorTurret::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	if ( cent_pEnemy->m_iClassname == m_iClassname )
		return false;
	...
	Vector vEnemyPos = cent_pEnemy->EyePosition();
...
bool CNPC_FloorTurret::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
	...
	{
		CEntity *cent_pEnemy = CEntity::Instance(pEnemy);
		if ( cent_pEnemy->Classify() == CLASS_PLAYER_ALLY_VITAL )
			return false;
	} 
...
bool CNPC_FloorTurret::PreThink( turretState_e state )
{
	...
	if ( !IsBeingCarriedByPlayer() )
	{
		if ( OnSide() == false )
		{
			...
			{
				m_hLastNPCToKickMe->DispatchInteraction( g_interactionTurretStillStanding, NULL, BaseEntity() );
				m_hLastNPCToKickMe.Set(NULL);
			}
		}
		else
...
void CNPC_FloorTurret::SetEyeState( eyeState_t state )
{
	...
	if ( !m_hEyeGlow )
	{
		...
		CE_CSprite *sprite = CE_CSprite::SpriteCreate( FLOOR_TURRET_GLOW_SPRITE, GetLocalOrigin(), false );
		if ( !sprite )
			return;

		m_hEyeGlow.Set(sprite->BaseEntity());
		sprite->SetTransparency( kRenderWorldGlow, 255, 0, 0, 128, kRenderFxNoDissipation );
		sprite->SetAttachment( BaseEntity(), m_iEyeAttachment );
	}
	...
	{
		CE_CBeam *beam = CE_CBeam::BeamCreate( LASER_BEAM_SPRITE, 1.0f );
		if ( beam == NULL )
			return;

		m_hLaser.Set(beam->BaseEntity());
		beam->EntsInit( this, this );
		beam->FollowEntity( BaseEntity() );
		beam->SetStartAttachment( LookupAttachment( "laser_start" ) );
		beam->SetEndAttachment( LookupAttachment( "laser_end" ) );
		beam->SetNoise( 0 );
		beam->SetColor( 255, 0, 0 );
		beam->SetScrollRate( 0 );
		beam->SetWidth( 1.0f );
		beam->SetEndWidth( 1.0f );
		beam->SetBrightness( 160 );
		beam->SetBeamFlags( SF_BEAM_SHADEIN );
...
int CNPC_FloorTurret::VPhysicsTakeDamage( const CTakeDamageInfo &info )
{
	...
	{
		bShouldIgnoreFromFront = FClassnameIs( CEntity::Instance(info.GetInflictor()), "crossbow_bolt" );
	}
...
{
	CCombatCharacter *pBCC = CEntity::Instance(pVictim)->MyCombatCharacterPointer();
...
// Purpose: 
//-----------------------------------------------------------------------------
Vector CNPC_FloorTurret::GetAttackSpread( CBaseEntity *pWeapon, CBaseEntity *pTarget ) 
{
	...
	if ( cent_pTarget )
	{
		if ( cent_pTarget->Classify() == CLASS_PLAYER || cent_pTarget->Classify() == CLASS_ANTLION || cent_pTarget->Classify() == CLASS_ZOMBIE )
		{
			// Make me much more accurate
			weaponProficiency = WEAPON_PROFICIENCY_PERFECT;
		}
		else if ( cent_pTarget->Classify() == CLASS_COMBINE )
		{
	...
	return VECTOR_CONE_10DEGREES * ((CCombatWeapon::GetDefaultProficiencyValues())[ weaponProficiency ].spreadscale);
}

//------------------------------------------------------------------------------
// Do we have a physics attacker?
//------------------------------------------------------------------------------
CBaseEntity *CNPC_FloorTurret::HasPhysicsAttacker( float dt )
{
...
void CNPC_FloorTurret::BreakThink( void )
{
	...
	// K-boom
	RadiusDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), 15.0f, DMG_BLAST ), vecOrigin, (10*12), CLASS_NONE, BaseEntity() );
	...
	PropBreakableCreateAll( GetModelIndex(), VPhysicsGetObject(), params, BaseEntity(), -1, true );
...
void CNPC_FloorTurret::SelfDestructThink( void )
{
	...
	if ( gpGlobals->curtime > ( m_flPingTime + flBeepTime ) )
	{
		...
		params.m_nPitch = (int)floor( flBeepPitch );
...
LINK_ENTITY_TO_CUSTOM_CLASS( floorturret_tipcontroller, info_target, CTurretTipController );
