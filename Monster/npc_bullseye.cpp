...
class CBullseyeList : public CBaseGameSystem
{
public:
	CBullseyeList( char const *name ) : CBaseGameSystem( name )
	{
	}
...
static CBullseyeList g_BullseyeList( "CBullseyeList" );
...
BEGIN_DATADESC( CNPC_Bullseye )
	...
	DEFINE_KEYFIELD( _m_flFieldOfView, FIELD_FLOAT, "minangle" ),
...
CNPC_Bullseye::CNPC_Bullseye()
{
	...
	_m_flFieldOfView = 360.0f;
	m_fAutoaimRadius = 0.0f;
...
void CNPC_Bullseye::Spawn( void )
{
	...
	m_iHealth		= sk_bullseye_health.GetInt();
	m_flFieldOfView = _m_flFieldOfView;
...
void CNPC_Bullseye::DecalTrace( trace_t *pOldTrace, char const *decalName )
{
	...
	trace_t pNewTrace;
	UTIL_TraceLine(vStartTrace, vEndTrace, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &pNewTrace);
...
void CNPC_Bullseye::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
	...
	CEntity	*pEntity = CEntity::Instance(pNewTrace.m_pEnt);
...
bool CNPC_Bullseye::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
	...
	{
		float distSq = ( GetAbsOrigin().AsVector2D() - cent->GetAbsOrigin().AsVector2D() ).LengthSqr();
...
void CNPC_Bullseye::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	{
		CEntity *cent = CEntity::Instance(info.GetAttacker());
		CAI_NPC *pInstigator = cent->MyNPCPointer();
