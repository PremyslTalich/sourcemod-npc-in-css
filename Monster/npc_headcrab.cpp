...
void CBaseHeadcrab::OnChangeActivity( Activity NewActivity )
{
	...
	if( fRandomize )
	{
		SetCycle( enginerandom->RandomFloat( 0.0, flRandomRange ) );
	}
}
...
bool CBaseHeadcrab::IsFirmlyOnGround()
{
	...
	trace_t tr;
	UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() - Vector( 0, 0, HEADCRAB_MAX_LEDGE_HEIGHT ), MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), &tr );
...
void CBaseHeadcrab::HandleAnimEvent( animevent_t *pEvent )
{
	...
	CAI_NPC::HandleAnimEvent( pEvent );
}
...
void CBaseHeadcrab::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_HEADCRAB_WAIT_FOR_BARNACLE_KILL:
			...
			{
				m_flNextFlinchTime = gpGlobals->curtime + enginerandom->RandomFloat( 1.0f, 2.0f );
		...
		case TASK_HEADCRAB_DROWN:
			...
			{
				OnTakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), (float)((*m_iHealth) * 2), DMG_DROWN ) );
			}
			break;
		...
		case TASK_HEADCRAB_CEILING_FALL:
			{
				...
				UTIL_TraceHull( vecPrPos, vecPrPos, GetHullMins(), GetHullMaxs(), MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CBaseHeadcrab::LeapTouch( CEntity *pOther )
{
	...
	if ( IRelationType( pOther->BaseEntity() ) == D_HT )
	{
...
void CBaseHeadcrab::GatherConditions( void )
{
	...
	if ( m_bHidden )
	{
		...
		trace_t tr;
		UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin(),GetHullMins(), GetHullMaxs(), MASK_SHOT, BaseEntity(), GetCollisionGroup(), &tr );
		if ( tr.fraction == 1.0 )
		{
	...
	CEntity *ground = GetGroundEntity();
...
void CBaseHeadcrab::PrescheduleThink( void )
{
	...
	//
	if (( m_NPCState == NPC_STATE_COMBAT ) && ( enginerandom->RandomFloat( 0, 5 ) < 0.1 ))
	{
		IdleSound();
	}
...
void CBaseHeadcrab::BeginClimbFromCanister()
{
	...
	CE_AI_Hint *pHint = CAI_HintManager::FindHint( this, HINT_HEADCRAB_BURROW_POINT, 0, HEADCRAB_BURROW_POINT_SEARCH_RADIUS, &vecSearchCenter );
	...
	else
	{
		...
		m_nJumpFromCanisterDir = enginerandom->RandomInt( 0, 2 );
	}
...
void CBaseHeadcrab::JumpFromCanister()
{
	...
	CEntity *pEnemy = CEntity::Instance(BestEnemy());
	if ( pEnemy )
	{
...
void CBaseHeadcrab::StartTask( const Task_t *pTask )
{
	...
	case TASK_HEADCRAB_CEILING_POSITION:
		{
			trace_t tr;
			UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0, 512 ), NAI_Hull::Mins( GetHullType() ), NAI_Hull::Maxs( GetHullType() ), MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr );
	...
	case TASK_HEADCRAB_HARASS_HOP:
		{
			...
			if ( GetEnemy() )
			{
				...
				m_vecCommittedJumpPos += up * enginerandom->RandomFloat( 80, 150 );
				m_vecCommittedJumpPos += forward * enginerandom->RandomFloat( 32, 80 );
	...
	case TASK_HEADCRAB_HOP_OFF_NPC:
		{
			...
			if( ground )
			{
				...
				if( ground && ground->VPhysicsGetObject() )
				{
					...
					{
						g_helpfunc.NPCPhysics_CreateSolver( BaseEntity(), ground->BaseEntity(), true, 0.5 );
					}
				}
...
int CBaseHeadcrab::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if( pEnemy )
	{
		...
		if ( tr.m_pEnt != GetEnemy()->BaseEntity() )
		{
		...
		if( GetEnemy()->EyePosition().z - 36.0f > GetAbsOrigin().z )
		{
			...
			UTIL_TraceHull( vStartHullTrace, vEndHullTrace,GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), &tr );

			if ( tr.m_pEnt != NULL && tr.m_pEnt != GetEnemy()->BaseEntity() )
			{
				return COND_TOO_CLOSE_TO_ATTACK;
			}
		}
	...
	if (m_NPCState == NPC_STATE_DEAD) 
	{
		if (GetAbsVelocity().Length() > 250)
		{
			...
			UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecDir * 100, 
				MASK_SOLID_BRUSHONLY, BaseEntity(), COLLISION_GROUP_NONE, &tr); 
			float dotPr = DotProduct(vecDir,tr.plane.normal);
			if ((tr.fraction						!= 1.0) && 
				(dotPr <  -0.8) )
			{
				CTakeDamageInfo	info( GetWorldEntity()->BaseEntity(), GetWorldEntity()->BaseEntity(), 100.0f, DMG_CRUSH );
...
#define HEADCRAB_SCORCH_FLOOR	30
	...
	return CAI_NPC::OnTakeDamage_Alive( info );
}
...
int CBaseHeadcrab::SelectSchedule( void )
{
	...
	switch ( m_NPCState )
	{
		case NPC_STATE_ALERT:
		{
			if (HasCondition( COND_LIGHT_DAMAGE ) || HasCondition( COND_HEAVY_DAMAGE ))
			{
				...
				{
					m_flNextFlinchTime = gpGlobals->curtime + enginerandom->RandomFloat( 1, 3 );
	...
	CE_AI_Hint *phint = (CE_AI_Hint *)GetHintNode();
	if ( phint && phint->HintType() == HINT_HEADCRAB_BURROW_POINT )
	{
...
bool CBaseHeadcrab::ValidBurrowPoint( const Vector &point )
{
	...
	UTIL_TraceHull( point, point+Vector(0,0,1), GetHullMins(), GetHullMaxs(), 
		MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), &tr );
...
void CBaseHeadcrab::CreateDust( bool placeDecal )
{
	...
	if ( tr.fraction < 1.0f )
	{
		...
		{
			UTIL_CreateDust(tr.endpos + Vector(0, 0, 24), GetLocalAngles(), 1.0f, 10.0f);
...
{
	if( IsOnFire() && enginerandom->RandomInt( 0, HEADCRAB_BURN_SOUND_FREQUENCY ) > 0 )
	{
...
void CFastHeadcrab::Spawn( void )
{
	...
	m_iHealth = sk_headcrab_fast_health.GetInt();
	...
	if( IsAlive() && GetNavigator()->IsGoalActive() )
	{
		...
		case HEADCRAB_RUNMODE_ACCELERATE:
			...
			if( m_flPlaybackRate >= HEADCRAB_RUN_MAXSPEED )
			{
				...
				m_flSlowRunTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.1, 1.0 );
			}
			break;
		...
		case HEADCRAB_RUNMODE_DECELERATE:
			...
			if( m_flPlaybackRate <= HEADCRAB_RUN_MINSPEED )
			{
				...
				m_flPauseTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.2, 0.5 );
	...
	case TASK_HEADCRAB_HOP_ASIDE:
		{
			...
			fJumpIsLeft = false;
			if( enginerandom->RandomInt( 0, 100 ) < 50 )
			{
				fJumpIsLeft = true;
				vecRight.Negate();
			}
			...
			UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + vecDir,GetHullMins(), GetHullMaxs(), MASK_SHOT, BaseEntity(), GetCollisionGroup(), &tr );
			...
			if( tr.fraction == 1.0 )
			{
				...
				GetMoveProbe()->MoveLimit( NAV_JUMP, GetAbsOrigin(), tr.endpos, MASK_NPCSOLID, enemy, &moveTrace );
...
bool CFastHeadcrab::QuerySeeEntity(CBaseEntity *pSightEnt, bool bOnlyHateOrFearIfNPC )
{
	...
	{
		CEntity *pSightEnt_cent = CEntity::Instance(pSightEnt);
		if( fabs( pSightEnt_cent->GetAbsOrigin().z - GetAbsOrigin().z ) >= 150 )
		{
...
void CBlackHeadcrab::Spawn( void )
{
	...
	m_bPanicState = false;
	m_iHealth = sk_headcrab_poison_health.GetInt();
...
int CBlackHeadcrab::SelectSchedule( void )
{
	...
	if ( !IsHangingFromCeiling() )
	{
		...
		if ( HasCondition( COND_LIGHT_DAMAGE ) || HasCondition( COND_HEAVY_DAMAGE ) )
		{
			...
			{
				m_flNextHopTime = gpGlobals->curtime + enginerandom->RandomFloat( 1, 3 );
				return SCHED_SMALL_FLINCH;
			}
		}
...
void CBlackHeadcrab::TouchDamage( CEntity *pOther )
{
	...
	{
		CTakeDamageInfo info;
		if ( CalcDamageInfo( &info ) >= pOther->GetHealth() )
			info.SetDamage( pOther->GetHealth() - 1 );

		pOther->TakeDamage( info  );

		if ( pOther->IsAlive() && pOther->GetHealth() > 1)
		{
			if ( hl2_episodic->GetBool() )
			{
				if ( pOther->IsPlayer() )
				{
					...
					pOther->TakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), pOther->GetHealth() - 1, DMG_POISON ) );
				}
				else
				{
					// Just take some amount of slash damage instead
					pOther->TakeDamage( CTakeDamageInfo( BaseEntity(), BaseEntity(), sk_headcrab_poison_npc_damage.GetFloat(), DMG_SLASH ) );
				}
			}
			else
			{
...
void CBlackHeadcrab::Eject( const QAngle &vecAngles, float flVelocityScale, CEntity *pEnemy )
{
	...
	if ( pEnemy )
	{
		SetEnemy( pEnemy->BaseEntity() );
		UpdateEnemyMemory(pEnemy->BaseEntity(), pEnemy->GetAbsOrigin());
	}
	...
	SetAbsVelocity( flVelocityScale * enginerandom->RandomInt( 20, 50 ) * 
		Vector( enginerandom->RandomFloat( -1.0, 1.0 ), enginerandom->RandomFloat( -1.0, 1.0 ), enginerandom->RandomFloat( 0.5, 1.0 ) ) );
...
void CBlackHeadcrab::EjectTouch( CEntity *pOther )
{
	...
	{
		// Keep trying to take cover for at least a few seconds.
		Panic( enginerandom->RandomFloat( 2, 8 ) );
	}
}
...
void CBlackHeadcrab::JumpFlinch( const Vector *pvecDir )
{
	...
	if (pvecDir)
	{
		SetAbsVelocity( Vector( pvecDir->x * 4, pvecDir->y * 4, up.z ) * enginerandom->RandomFloat( 40, 80 ) );
	}
	else
	{
		SetAbsVelocity( Vector( enginerandom->RandomFloat( -4, 4 ), enginerandom->RandomFloat( -4, 4 ), up.z ) * enginerandom->RandomFloat( 40, 80 ) );
	}
}
...
void CBlackHeadcrab::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_POISONHEADCRAB_FLINCH_HOP )
	{
		...
		Panic( enginerandom->RandomFloat( 2, 5 ) );
