...
{
	CE_DECLARE_CLASS( CNPCSpawnDestination, CPointEntity );

public:
	CNPCSpawnDestination();
	bool IsAvailable();						// Is this spawn destination available for selection?
	void OnSpawnedNPC( CAI_NPC *pNPC );	// Notify this spawn destination that an NPC has spawned here.
...
abstract_class CBaseNPCMaker : public CE_InfoTarget_Fix
{
public:
	CE_DECLARE_CLASS( CBaseNPCMaker, CE_InfoTarget_Fix );
	CE_CUSTOM_ENTITY();
	...
	virtual	void ChildPreSpawn( CAI_NPC *pChild ) {};
	virtual	void ChildPostSpawn( CAI_NPC *pChild );
...
{
public:
	CE_DECLARE_CLASS( CNPCMaker, CBaseNPCMaker );
...
{
public:
	CE_DECLARE_CLASS( CTemplateNPCMaker, CBaseNPCMaker );
...
protected:
	virtual void PrecacheTemplateEntity( CEntity *pEntity );

	bool PlaceNPCInRadius( CAI_NPC *pNPC );
	bool PlaceNPCInLine( CAI_NPC *pNPC );
