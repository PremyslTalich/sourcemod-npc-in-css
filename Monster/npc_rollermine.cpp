...
ConVar	sk_rollermine_bullet_hit( "sk_rollermine_bullet_hit","8");
...
//=========================================================
//=========================================================
class CNPC_RollerMine : public CNPCBaseInteractive<CE_Cycler_Fix>
{
public:
	...
	void	SpikeTouch( CEntity *pOther );
	void	ShockTouch( CEntity *pOther );
	void	CloseTouch( CEntity *pOther );
	void	EmbedTouch( CEntity *pOther );

	void	CQ_EmbedTouch( CBaseEntity *pOther );
	...
	bool	IsPlayerVehicle( CEntity *pEntity );
	...
	Vector EyePosition() 
	{
		...
		Vector eye = CollisionProp_Actual()->GetCollisionOrigin();
		eye.z += CollisionProp_Actual()->OBBMaxs().z;
		return eye;
	}
	...
	float	VehicleHeading( CEntity *pVehicle );

	NPC_STATE SelectIdealState();

	// Vehicle sticking
	void		StickToVehicle( CEntity *pOther );
	void		AnnounceArrivalToOthers( CEntity *pOther );
	void		UnstickFromVehicle( void );
	CEntity		*GetVehicleStuckTo( void );
...
CEntity *NPC_Rollermine_DropFromPoint( const Vector &originStart, CBaseEntity *pOwner, const char *pszTemplate )
{
	...
	{
		g_helpfunc.MapEntity_ParseEntity( pEntity, pszTemplate, NULL );
...
{
	UTIL_TraceHull( GetAbsOrigin() + Vector(0,0,64), GetAbsOrigin() - Vector( 0, 0, MAX_TRACE_LENGTH ), Vector(-16,-16,-16), Vector(16,16,16), MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), tr );
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RollerMine::WakeupMine( CAI_NPC *pNPC )
{
...
void CNPC_RollerMine::WakeNeighbors()
{
	...
	if ( GetSquad() )
	{
		...
		{
			WakeupMine( (CAI_NPC *)CEntity::Instance(pSquadMember) );
		}
		return;
	}

	CEntity *entityList[64];
	...
	{
		boxCount--;
		CAI_NPC *pNPC = entityList[boxCount]->MyNPCPointer();
		if ( WakeupMine( pNPC ) )
		{
			wakeCount++;
			if ( wakeCount >= 2 )
...
{
	g_pSM->LogError(myself, "CNPC_RollerMine::OnRestore");
	BaseClass::OnRestore();
	...
	{
		if ( !g_CEventQueue->HasEventPending( BaseEntity(), "JoltVehicle" ) )
		{
			g_CEventQueue->AddEvent( BaseEntity(), "JoltVehicle", RandomFloat(3,6), NULL, NULL );
		}
	}
}
...
void CNPC_RollerMine::RunAI()
{
	if( m_bTurnedOn )
	{
		...
		if( !m_bHeld && vecVelocity.Length() > 64.0 )
		{
			...
			{
				// Scare combine
				g_helpfunc.CSoundEnt_InsertSound((SOUND_DANGER | SOUND_CONTEXT_COMBINE_ONLY | SOUND_CONTEXT_REACT_TO_SOURCE | SOUND_CONTEXT_DANGER_APPROACH), WorldSpaceCenter() + Vector( 0, 0, 32 ) + vecVelocity * 0.5f, 120, 0.2f, BaseEntity(), SOUNDENT_CHANNEL_REPEATED_DANGER );
			}
			else
			{
				// Scare player allies
				g_helpfunc.CSoundEnt_InsertSound( (SOUND_DANGER | SOUND_CONTEXT_EXCLUDE_COMBINE | SOUND_CONTEXT_REACT_TO_SOURCE | SOUND_CONTEXT_DANGER_APPROACH), WorldSpaceCenter() + Vector( 0, 0, 32 ) + vecVelocity * 0.5f, 120, 0.2f, BaseEntity(), SOUNDENT_CHANNEL_REPEATED_DANGER );
			}
		}
...
int CNPC_RollerMine::GetHackedIdleSchedule( void )
{
	...
	CEntity *pPlayer = g_helpfunc.FindEntityByName((CBaseEntity *)NULL, "!player");
	if ( !pPlayer )
		return SCHED_NONE;
...
bool CNPC_RollerMine::QuerySeeEntity(CBaseEntity *pSightEnt, bool bOnlyHateOrFearIfNPC)
{
	if ( IRelationType( pSightEnt ) == D_FR )
	{
		...
		float flDist = (WorldSpaceCenter() - cent->WorldSpaceCenter()).LengthSqr();
...
void CNPC_RollerMine::StartTask( const Task_t *pTask )
{
	...
	case TASK_ROLLERMINE_UNBURROW:
		
		{
			...
			if ( tr.fraction < 1.0f )
			{
				UTIL_CreateDust(tr.endpos + Vector(0,0,24), GetLocalAngles(), 6.0f, 10.0f);
			}
		}
	...
	case TASK_ROLLERMINE_GET_PATH_TO_FLEE:
		{
			...
			CFakeHandle hNearestEnemy;
			AIEnemiesIter_t iter;
			for( AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst( &iter ); pEMemory != NULL; pEMemory = GetEnemies()->GetNext( &iter ) )
			{
				CBaseEntity *cbase = pEMemory->hEnemy;
				CEntity *pEnemy = CEntity::Instance(cbase);
				if ( !pEnemy || !pEnemy->IsAlive() )
					continue;
				if ( IRelationType( cbase ) != D_FR )
					continue;			
	...
	case TASK_ROLLERMINE_NUDGE_TOWARDS_NODES:
		{
			...
			if( pPhysicsObject )
			{
				...
				for( int i = 0 ; i < 4 ; i++ )
				{
					...
					x = enginerandom->RandomInt( -1, 1 );
					y = enginerandom->RandomInt( -1, 1 );
					...
					UTIL_TraceLine( WorldSpaceCenter(), WorldSpaceCenter() + vecNudge * 24.0f, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CNPC_RollerMine::RunTask( const Task_t *pTask )
{
	...
	case TASK_ROLLERMINE_UNBURROW:
		{	
			...
			trace_t	tr;
			UTIL_TraceHull( vCenter, vCenter, Vector(-16,-16,-16), Vector(16,16,16), MASK_NPCSOLID, BaseEntity(), GetCollisionGroup(), &tr );
	...
	case TASK_ROLLERMINE_CHARGE_ENEMY:
		{
			...
			if ( EnemyInVehicle() )
			{
				CCombatCharacter *pCCEnemy = pEnemy->MyCombatCharacterPointer();
				float flT;

				CEntity *cent_vehicle = CEntity::Instance(pCCEnemy->GetVehicleEntity());
				// Project it's velocity and find our closest point on that line. Do it all in 2d space.
				Vector vecVehicleVelocity = cent_vehicle->GetSmoothedVelocity();
	...
	case TASK_ROLLERMINE_POWERDOWN:
		{
			if ( m_flPowerDownTime <= gpGlobals->curtime )
			{
				...
				g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, GetAbsOrigin(), 400, 0.5f, BaseEntity() );
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_RollerMine::SpikeTouch( CEntity *pOther )
{
	/*
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::CloseTouch( CEntity *pOther )
{
	if ( pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS) )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::EmbedTouch( CEntity *pOther )
{
	if ( pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS) )
		return;
	...
	if ( pOther->entindex() == 0 )
	{
		...
		// Make some dust
		UTIL_CreateDust( tr.endpos, GetLocalAngles(), 3.0f, 10.0f );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_RollerMine::IsPlayerVehicle( CEntity *pEntity )
{
	IServerVehicle *pVehicle = pEntity->GetServerVehicle();
	if ( pVehicle )
	{
		CPlayer *pPlayer = ToBasePlayer( CEntity::Instance(pVehicle->GetPassenger()) );
		if ( pPlayer != NULL )
		{
			Disposition_t disp = IRelationType(pPlayer->BaseEntity());
...
float CNPC_RollerMine::GetAttackDamageScale( CBaseEntity *pVictim )
{
	...
	if ( HasSpawnFlags( SF_ROLLERMINE_FRIENDLY ) )
	{
		...
		if ( cent->MyNPCPointer() )
		{
			...
			if ( pPlayer && cent->MyNPCPointer()->IRelationType( pPlayer->BaseEntity() ) != D_HT )
				return 0.0;
		}
	}
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::ShockTarget( CEntity *pOther )
{
	CE_CBeam *pBeam;

	if( m_bHackedByAlyx )
	{
		pBeam = CE_CBeam::BeamCreate( "sprites/rollermine_shock_yellow.vmt", 4 );
	}
	else
	{
		pBeam = CE_CBeam::BeamCreate( "sprites/rollermine_shock.vmt", 4 );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::NotifyInteraction( CAI_NPC *pUser )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::ShockTouch( CEntity *pOther )
{
	if ( pOther->IsSolidFlagSet(FSOLID_TRIGGER | FSOLID_VOLUME_CONTENTS) )
		return;
	...
	CTakeDamageInfo	info( BaseEntity(), BaseEntity(), vecForce, out, sk_rollermine_shock.GetFloat(), DMG_SHOCK );
	...
	if( FClassnameIs( pOther, "npc_combine_s" ) )
	{
		if( pOther->GetHealth() <= (pOther->GetMaxHealth() / 2) ) 
		{
			...
			pOther->MyCombatCharacterPointer()->BecomeRagdollBoogie( BaseEntity(), vecDamageForce, 5.0f, SF_RAGDOLL_BOOGIE_ELECTRICAL );
			return;
		}
		else
		{
...
void CNPC_RollerMine::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( pEvent->deltaCollisionTime < 0.5 && (pOther == BaseEntity()) )
		return;


	BaseClass::VPhysicsCollision( index, pEvent );

	CEntity *cent_pOther = CEntity::Instance(pOther);

	// If we've just hit a vehicle, we want to stick to it
	if ( m_bHeld || m_hVehicleStuckTo || !IsPlayerVehicle( cent_pOther ) )
	{
		...
		if ( m_bEmbedOnGroundImpact )
		{
			...
			VALVE_ENTITYFUNCPTR valve_touch;
			void (CEntity::*cent_touch)(CBaseEntity *) = static_cast <void (CEntity::*)(CBaseEntity *)> (&CNPC_RollerMine::CQ_EmbedTouch);
			memcpy(&valve_touch, &cent_touch, sizeof(void *));
			g_PostSimulationQueue->QueueCall( BaseEntity(), valve_touch, pOther );
		}
		return;
	}

	StickToVehicle( cent_pOther );
}


void CNPC_RollerMine::CQ_EmbedTouch( CBaseEntity *pOther )
{
	CNPC_RollerMine *cent = (CNPC_RollerMine *)CEntity::Instance(reinterpret_cast<CBaseEntity *>(this));
	if(cent)
	{
		cent->EmbedTouch(CEntity::Instance(pOther));
	}
}
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::StickToVehicle( CEntity *pOther )
{
	...
	m_hVehicleStuckTo.Set(pOther->BaseEntity());
...
int CNPC_RollerMine::CountRollersOnMyVehicle( CUtlVector<CNPC_RollerMine*> *pRollerList )
{
	...
	{
		CAI_NPC *pNPC = entityList[i]->MyNPCPointer();
		if ( pNPC && *(pNPC->m_iClassname) == *(m_iClassname) && pNPC != this )
		{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_RollerMine::AnnounceArrivalToOthers( CEntity *pOther )
{
	...
	*/
	...
	if ( iRollers > 1 )
	{
		...
		{
			variant_t emptyVariant;
			g_CEventQueue->AddEvent( aRollersOnVehicle[i]->BaseEntity(), "RespondToChirp", RandomFloat(2,3), NULL, NULL );
		}
	}
//	}
}
...
void CNPC_RollerMine::UnstickFromVehicle( void )
{
	...
	g_CEventQueue->CancelEventOn( BaseEntity(), "JoltVehicle" );
...
// Purpose: 
//-----------------------------------------------------------------------------
CEntity *CNPC_RollerMine::GetVehicleStuckTo( void )
{
	if ( !m_pConstraint )
		return NULL;
...
int CNPC_RollerMine::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	{
		m_iBulletHit++;
		if(m_iBulletHit == sk_rollermine_bullet_hit.GetInt())
		{
			SetThink( &CNPC_RollerMine::PreDetonate );
			SetNextThink( gpGlobals->curtime + enginerandom->RandomFloat( 0.1f, 0.5f ) );
		}
		return 0;
	}
	...
	if ( info.GetDamageType() & (DMG_BURN|DMG_BLAST) )
	{
		...
		if ( attacker && *(attacker->m_iClassname) != *(m_iClassname))
		{
			SetThink( &CNPC_RollerMine::PreDetonate );
			SetNextThink( gpGlobals->curtime + enginerandom->RandomFloat( 0.1f, 0.5f ) );
		}
...
void CNPC_RollerMine::Explode( void )
{
	...
	{
		ExplosionCreate( WorldSpaceCenter(), GetLocalAngles(), BaseEntity(), (int)expDamage, 128, true );
	}

	CTakeDamageInfo	info( BaseEntity(), BaseEntity(), 1, DMG_GENERIC );
	Event_Killed( info );
...
void CNPC_RollerMine::UpdateRollingSound()
{
	...
	case ROLL_SOUND_CLOSED:
		CEntity::GetParametersForSound( "NPC_RollerMine.Roll", params, NULL );
		break;
	case ROLL_SOUND_OPEN:
		CEntity::GetParametersForSound( "NPC_RollerMine.RollWithSpikes", params, NULL );
		break;
	...
	if ( m_rollingSoundState != soundState )
	{
		...
		m_pRollSound = g_SoundController->SoundCreate( filter, entindex(), params.channel, params.soundname, params.soundlevel );
		g_SoundController->Play( m_pRollSound, params.volume, params.pitch );
		m_rollingSoundState = soundState;
	}
	...
	if ( m_pRollSound )
	{
		...
		g_SoundController->SoundChangePitch( m_pRollSound, params.pitchlow + (params.pitchhigh - params.pitchlow) * rollingSpeed, 0.1 );
		g_SoundController->SoundChangeVolume( m_pRollSound, params.volume * rollingSpeed, 0.1 );
	}
}
...
{
	g_SoundController->SoundDestroy( m_pRollSound );
	m_pRollSound = NULL;
}
...
void CNPC_RollerMine::UpdatePingSound()
{
	...
	if ( pingSpeed > 0 )
	{
		...
		CSoundParameters params;
		CEntity::GetParametersForSound( "NPC_RollerMine.Ping", params, NULL );
		if ( !m_pPingSound )
		{
			CPASAttenuationFilter filter( this );
			m_pPingSound = g_SoundController->SoundCreate( filter, entindex(), params.channel, params.soundname, params.soundlevel );
			g_SoundController->Play( m_pPingSound, params.volume, 101 );
		}

		g_SoundController->SoundChangePitch( m_pPingSound, params.pitchlow + (params.pitchhigh - params.pitchlow) * pingSpeed, 0.1 );
		g_SoundController->SoundChangeVolume( m_pPingSound, params.volume, 0.1 );
		//DevMsg("PING: %.1f\n", pingSpeed );
...
{
	g_SoundController->SoundDestroy( m_pPingSound );
	m_pPingSound = NULL;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_RollerMine::IsValidEnemy( CBaseEntity *_pEnemy )
{
	CEntity *pEnemy = CEntity::Instance(_pEnemy);
	...
	if ( pEnemy )
	{
		...
		if ( flDistance >= m_flSeeVehiclesOnlyBeyond )
		{
			...
			if ( pCCEnemy != NULL && pCCEnemy->IsInAVehicle() )
			{
				...
				if ( m_bBuried )
					return ( VehicleHeading( CEntity::Instance(pCCEnemy->GetVehicle()->GetVehicleEnt()) ) > DOT_20DEGREE );

				// If we're not buried, chase him as long as he's not heading away from us
				return ( VehicleHeading( CEntity::Instance(pCCEnemy->GetVehicleEntity()) ) > 0 );
			}
	...
	return BaseClass::IsValidEnemy( _pEnemy );
}
...
bool CNPC_RollerMine::EnemyInVehicle( void )
{
	...
	CCombatCharacter *pCCEnemy = GetEnemy()->MyCombatCharacterPointer();
	if ( pCCEnemy != NULL && pCCEnemy->IsInAVehicle() )
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_RollerMine::VehicleHeading( CEntity *pVehicle )
{
