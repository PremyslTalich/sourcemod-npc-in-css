...
static void DispatchActivate( CEntity *pEntity )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCSpawnDestination::OnSpawnedNPC( CAI_NPC *pNPC )
{
	// Rename the NPC
...
CBaseNPCMaker::CBaseNPCMaker()
{
	...
	m_nMaxLiveChildren = 10;
	m_bDisabled = true;
	m_iszIngoreEnt = NULL_STRING;
}
...
bool CBaseNPCMaker::HumanHullFits( const Vector &vecLocation )
{
	...
	UTIL_TraceHull( vecLocation,
					...
					MASK_NPCSOLID,
					(m_hIgnoreEntity) ? m_hIgnoreEntity->BaseEntity() : NULL,
...
bool CBaseNPCMaker::CanMakeNPC( bool bIgnoreSolidEntities )
{
	...
	{
		CEntity *cent = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszIngoreEnt );
		m_hIgnoreEntity.Set((cent)?cent->BaseEntity():NULL);
	}
	...
	if ( HasSpawnFlags( SF_NPCMAKER_HIDEFROMPLAYER ) )
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pPlayer )
			{
				...
				if( pPlayer->FInViewCone_Vector( GetAbsOrigin() ) && pPlayer->FVisible_Vector( GetAbsOrigin() ) )
				{
...
void CNPCMaker::MakeNPC( void )
{
	...
	CAI_NPC *pent = (CAI_NPC*)CreateEntityByName( STRING(m_iszNPCClassname) );
	...
	DispatchSpawn( cbase );
	pent->SetOwnerEntity( BaseEntity() );
...
// Input  : *pChild - 
//-----------------------------------------------------------------------------
void CBaseNPCMaker::ChildPostSpawn( CAI_NPC *pChild )
{
	...
	{
		trace_t tr;
		UTIL_TraceHull( pChild->GetAbsOrigin(), pChild->GetAbsOrigin(), pChild->WorldAlignMins(), pChild->WorldAlignMaxs(), MASK_NPCSOLID, pChild->BaseEntity(), COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction != 1.0 && tr.m_pEnt )
		{
			CEntity *cent = CEntity::Instance(tr.m_pEnt);
			if ( FClassnameIs( cent, "prop_physics" ) )
			{
				// Set to non-solid so this loop doesn't keep finding it
				cent->AddSolidFlags( FSOLID_NOT_SOLID );
				g_helpfunc.UTIL_RemoveImmediate( tr.m_pEnt );
				continue;
			}
		}
...
BEGIN_DATADESC( CTemplateNPCMaker )
	...
	DEFINE_KEYFIELD( local_m_flRadius, FIELD_FLOAT, "radius" ),
...
CTemplateNPCMaker::CTemplateNPCMaker( void ) 
{
	m_iMinSpawnDistance = 0;
	local_m_flRadius = 0;
	m_iszTemplateName = NULL_STRING;
	m_iszDestinationGroup = NULL_STRING;
...
CNPCSpawnDestination *CTemplateNPCMaker::FindSpawnDestination()
{
	...
	pEnt = g_helpfunc.FindEntityByName( (CBaseEntity *)NULL, m_iszDestinationGroup );

	if( !pEnt )
	{
		DevWarning("Template NPC Spawner () doesn't have any spawn destinations!\n");
		return NULL;
	}
	...
	while( pEnt )
	{
		...
		if( pDestination && pDestination->IsAvailable() )
		{
			...
			if( m_CriterionVisibility != TS_YN_DONT_CARE )
			{
				...
				vecTopOfHull.x = 0;
				vecTopOfHull.y = 0;
				bool fVisible = (pPlayer->FVisible_Vector( vecTest ) || pPlayer->FVisible_Vector( vecTest + vecTopOfHull ) );
		...
		pEnt = g_helpfunc.FindEntityByName( pEnt->BaseEntity(), m_iszDestinationGroup );
	}
...
void CTemplateNPCMaker::MakeNPC( void )
{
	...
	if ( local_m_flRadius && HasSpawnFlags(SF_NPCMAKER_ALWAYSUSERADIUS) )
	{
		MakeNPCInRadius();
		return;
	}
...
//-----------------------------------------------------------------------------
bool CTemplateNPCMaker::PlaceNPCInLine( CAI_NPC *pNPC )
{
	Vector vecPlace;
	Vector vecLine;
	...
	trace_t tr;
	UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() - Vector( 0, 0, 8192 ), MASK_SHOT, pNPC->BaseEntity(), COLLISION_GROUP_NONE, &tr );
	vecPlace = tr.endpos;
...
void CTemplateNPCMaker::MakeNPCInRadius( void )
{
	...
	if ( !PlaceNPCInRadius( pent ) )
	{
		...
		g_helpfunc.UTIL_RemoveImmediate( pent->BaseEntity() );
		return;
	}
...
//-----------------------------------------------------------------------------
bool CTemplateNPCMaker::PlaceNPCInRadius( CAI_NPC *pNPC )
{
	Vector vPos;

	if ( CAI_NPC::FindSpotForNPCInRadius( &vPos, GetAbsOrigin(), pNPC, local_m_flRadius ) )
	{
...
{
	bool bInRadius = ( m_iszDestinationGroup == NULL_STRING && local_m_flRadius > 0.1 );
	while ( nNPCs-- )
	{
		if ( !bInRadius )
		{
