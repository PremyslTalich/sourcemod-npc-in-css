...
void CNPC_Manhack::Event_Killed( const CTakeDamageInfo &info )
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		sparkPos.x += enginerandom->RandomFloat(-12,12);
		sparkPos.y += enginerandom->RandomFloat(-12,12);
		sparkPos.z += enginerandom->RandomFloat(-12,12);
	...
	if ( ( info.GetDamageType() & (DMG_CLUB|DMG_CRUSH|DMG_BLAST) ) || ( enginerandom->RandomInt( 0, 1 ) ) )
	{
		m_bGib = true;
	}
	else
...
void CNPC_Manhack::TakeDamageFromPhysicsImpact( int index, gamevcollisionevent_t *pEvent )
{
	...
	PhysCallbackDamage( BaseEntity(), CTakeDamageInfo( pHitEntity, pHitEntity, damageForce, damagePos, damage, damageType ), *pEvent, index );
...
void CNPC_Manhack::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	CEntity *pHitEntity = CEntity::Instance(cbase);
...
void CNPC_Manhack::VPhysicsShadowCollision( int index, gamevcollisionevent_t *pEvent )
{
	int otherIndex = !index;
	CBaseEntity *cbase = pEvent->pEntities[otherIndex];
	CEntity *pOther = CEntity::Instance(cbase);
...
void CNPC_Manhack::CreateSmokeTrail()
{
	...
	CSmokeTrail *pSmokeTrail =  CSmokeTrail::CreateSmokeTrail();
	if( !pSmokeTrail )
		return;
	...
	pSmokeTrail->m_StartColor.ptr->Init( 0.4f, 0.4f, 0.4f );
...
#define	MANHACK_PHYSICS_SEARCH_RADIUS	128
		...
		CEntity *pList[ MANHACK_PHYS_SEARCH_SIZE ];
...
bool CNPC_Manhack::CorpseGib( const CTakeDamageInfo &info )
{
	...
	if( info.GetDamageType() & DMG_CLUB )
	{
		...
		vecGibAVelocity.x = enginerandom->RandomFloat( -2000, 2000 );
		vecGibAVelocity.y = enginerandom->RandomFloat( -2000, 2000 );
		vecGibAVelocity.z = enginerandom->RandomFloat( -2000, 2000 );
	}
	else
	{
		...
		vecGibAVelocity.x = enginerandom->RandomFloat( -500, 500 );
		vecGibAVelocity.y = enginerandom->RandomFloat( -500, 500 );
		vecGibAVelocity.z = enginerandom->RandomFloat( -500, 500 );
	}
...
void CNPC_Manhack::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
	if( m_vNoiseMod.z == MANHACK_NOISEMOD_HIDE && !(m_spawnflags & SF_NPC_WAIT_FOR_SCRIPT) && !(m_spawnflags & SF_MANHACK_PACKED_UP) )
	{
		...
		float flNoiseMod = enginerandom->RandomFloat( 1.7, 2.3 );
...
int CNPC_Manhack::TranslateSchedule( int scheduleType ) 
{
	...
	case SCHED_ALERT_FACE:
		{
			if ( GetSquad() && m_bDoSwarmBehavior )
			{
...
bool CNPC_Manhack::OverrideMove( float flInterval )
{
	...
	{
		g_helpfunc.CSoundEnt_InsertSound( SOUND_DANGER, GetAbsOrigin(), 75, flInterval, BaseEntity() );
	}
	...
	else if (GetNavigator()->IsGoalActive())
	{
		...
		//NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 10 ), 255, 0, 0, true, 0.1);
  		if ( ProgressFlyPath( flInterval, GetEnemy_CBase(), MoveCollisionMask(), bReducible, strictTolerance ) == AINPP_COMPLETE )
			return true;
	}
	// -----------------------------------------------------------------
...
{
	float desYaw = enginerandom->RandomFloat(0,360);
...
void CNPC_Manhack::Splash( const Vector &vecSplashPos )
{
	...
	if ( !( contents & (CONTENTS_SLIME|CONTENTS_WATER)))
	{
		...
		trace_t	tr;
		UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() - Vector( 0, 0, 256 ), (CONTENTS_WATER|CONTENTS_SLIME), BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CNPC_Manhack::Slice( CEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	if ( IsHeldByPhyscannon() )
	{
		if ( pHitEntity && (pHitEntity->BaseEntity() == HasPhysicsAttacker( FLT_MAX )) )
			return;
	}
	...
	else if ( pHitEntity->IsNPC() && HasPhysicsAttacker( MANHACK_SMASH_TIME ))
	{
		...
		flDamage = 10; // the highest healthed fleshy enemy
	}
	else if ( dynamic_cast<CE_Prop*>(pHitEntity) || dynamic_cast<CE_CBreakable*>(pHitEntity) )
	{
		// If we hit a prop, we want it to break immediately
		flDamage = pHitEntity->GetHealth();
	}
	else if ( pHitEntity->IsNPC() && IRelationType( pHitEntity->BaseEntity() ) == D_HT  && FClassnameIs( pHitEntity, "npc_combine_s" ) ) 
	{
		flDamage *= 6.0f;
	}
	...
	CTakeDamageInfo info( BaseEntity(), BaseEntity(), flDamage, DMG_SLASH );

	// check for actual "ownership" of damage
	CBaseEntity *pPlayer = HasPhysicsAttacker( MANHACK_SMASH_TIME );
	if (pPlayer)
	{
	...
	if ( dir == vec3_origin )
	{
		dir = CEntity::Instance(tr.m_pEnt)->GetAbsOrigin() - GetAbsOrigin();
	}
	...
	if ( pHitEntity->BloodColor() == DONT_BLEED )
	{
		...
		g_helpfunc.DispatchEffect( "ManhackSparks", data );
...
void CNPC_Manhack::Bump( CEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	if (flInterval > 0)
	{
		...
		if (fabs( DotProduct( myUp, tr.plane.normal ) ) < 0.25 )
		{
			...
			// that we hit.
			trace_t	decalTrace;
			UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() - (tr.plane.normal * 24),MASK_SOLID, BaseEntity(), COLLISION_GROUP_NONE, &decalTrace );
	...
	if (GetNavigator()->IsGoalActive() && !(GetNavigator()->GetPath()->CurWaypointFlags() & bits_WP_TO_PATHCORNER) )
	{
		...
		if (IsMoveBlocked( moveTrace ) && 
			!CEntity::Instance(moveTrace.pObstruction)->ClassMatches( GetClassname() ))
		{
...
void CNPC_Manhack::CheckCollisions(float flInterval)
{
	...
	if ( IsHeldByPhyscannon() )
	{
		CEntity *pCarrier = CEntity::Instance(HasPhysicsAttacker( FLT_MAX ));
		if ( pCarrier )
		{
	...
	if ( (tr.fraction != 1.0 || tr.startsolid) && tr.m_pEnt)
	{
		...
		pHitEntity = CEntity::Instance(tr.m_pEnt);

		if( m_bHeld && pHitEntity->MyNPCPointer()/* && pHitEntity->MyNPCPointer()->IsPlayerAlly() */)
		{
...
void CNPC_Manhack::PlayFlySound(void)
{
	...
	{
		m_flNextEngineSoundTime	= gpGlobals->curtime + enginerandom->RandomFloat( 3.0, 10.0 );
...
void CNPC_Manhack::MoveExecute_Alive(float flInterval)
{
	...
	if (gpGlobals->curtime < m_flEngineStallTime)
	{
		...
		angles.x += -20+(enginerandom->RandomInt(-10,10));
		angles.z += -20+(enginerandom->RandomInt(0,40));
...
void CNPC_Manhack::MoveExecute_Dead(float flInterval)
{
	...
	{
//		UTIL_Smoke(GetAbsOrigin(), enginerandom->RandomInt(10, 15), 10);
		m_fSmokeTime = gpGlobals->curtime + enginerandom->RandomFloat( 0.1, 0.3);
	}
	...
	if (gpGlobals->curtime > m_fSparkTime)
	{
		...
		m_fSparkTime = gpGlobals->curtime + enginerandom->RandomFloat(0.1, 0.3);
	}
	...
	// ------------------------------------------
	angles.x += -20+(enginerandom->RandomInt(0,40));
	angles.z += -20+(enginerandom->RandomInt(0,40));
...
void CNPC_Manhack::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	if ( GetEnemy()->IsPlayer() && assert_cast< CPlayer * >(GetEnemy())->IsInAVehicle() )
	{
		flZDist = 24.0f;
	}
	...
	if ((GetAbsOrigin() - cent_pEnemy->GetAbsOrigin()).Length2D() > fl2DDist) 
	{
		SetCondition(COND_MANHACK_START_ATTACK);
	}
	else
	{
		float targetZ	= cent_pEnemy->EyePosition().z;
...
int CNPC_Manhack::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	bool bEnemyInVehicle = GetEnemy()->IsPlayer() && assert_cast< CPlayer * >(GetEnemy())->IsInAVehicle();
	if ( GetEnemy()->IsPlayer() && assert_cast< CPlayer * >(GetEnemy())->IsInAVehicle() )
	{
		flMinDist = 0;
		flMaxDist = 200.0f;
...
void CNPC_Manhack::Spawn(void)
{
	...
	m_iHealth			= sk_manhack_health.GetInt();
...
void CNPC_Manhack::StartEye( void )
{
	...
	if ( m_pEyeGlow == NULL )
	{
		m_pEyeGlow = CE_CSprite::SpriteCreate( MANHACK_GLOW_SPRITE, GetLocalOrigin(), false );
		m_pEyeGlow->SetAttachment( BaseEntity(), LookupAttachment( "Eye" ) );
	...
	{
		m_pLightGlow = CE_CSprite::SpriteCreate( MANHACK_GLOW_SPRITE, GetLocalOrigin(), false );
		m_pLightGlow->SetAttachment( BaseEntity(), LookupAttachment( "Light" ) );
...
void CNPC_Manhack::StartTask( const Task_t *pTask )
{
	...
	case TASK_GET_PATH_TO_ENEMY:
		{
			if (IsUnreachable(GetEnemy_CBase()))
			{
				TaskFail(FAIL_NO_ROUTE);
				return;
			}
	...
	case TASK_MANHACK_FIND_SQUAD_CENTER:
		{
			...
			AISquadIter_t iter;
			CAI_NPC *cent_pSquadMember;
			for (CBaseEntity *pSquadMember = GetSquad()->GetFirstMember( &iter ); pSquadMember; pSquadMember = GetSquad()->GetNextMember( &iter ) )
			{
				cent_pSquadMember = (CAI_NPC *)CEntity::Instance(pSquadMember);
				if (cent_pSquadMember->HasStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2 ))
				{
					m_vSavePosition = *(m_vSavePosition) + cent_pSquadMember->GetAbsOrigin() * 10;
					count += 10;
				}
				else
				{
					m_vSavePosition = *(m_vSavePosition) + cent_pSquadMember->GetAbsOrigin();
					count++;
				}
			}
			...
			if (GetEnemy() != NULL)
			{
				m_vSavePosition = *(m_vSavePosition) + GetEnemyLKP() * 4;
				count += 4;
			}
	...
	case TASK_MANHACK_FIND_SQUAD_MEMBER:
		{
			if (GetSquad() != NULL)
			{
				CBaseEntity *pSquadMember = GetSquad()->GetAnyMember();
				CAI_NPC *cent_pSquadMember = (CAI_NPC *)CEntity::Instance(pSquadMember);
				m_vSavePosition = cent_pSquadMember->GetAbsOrigin();

				// find attacking members
				AISquadIter_t iter;
				for (pSquadMember = GetSquad()->GetFirstMember( &iter ); pSquadMember; pSquadMember = GetSquad()->GetNextMember( &iter ) )
				{
					cent_pSquadMember = (CAI_NPC *)CEntity::Instance(pSquadMember);
					// are they attacking?
					if (cent_pSquadMember->HasStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2 ))
					...
					// do they have a goal?
					if (cent_pSquadMember->GetNavigator()->IsGoalActive())
					{
						m_vSavePosition = cent_pSquadMember->GetAbsOrigin();
						break;
					}
	...
	case TASK_MANHACK_MOVEAT_SAVEPOSITION:
		{
			trace_t tr;
			UTIL_TraceLine( GetAbsOrigin(), m_vSavePosition, MASK_NPCWORLDSTATIC, BaseEntity(), COLLISION_GROUP_NONE, &tr );
			if (tr.DidHitWorld())
			{
				TaskFail( FAIL_NO_ROUTE );
			}
...
bool CNPC_Manhack::IsInEffectiveTargetZone( CEntity *pTarget )
{
	...
	if ( pTarget && pTarget->IsPlayer() && assert_cast< CPlayer * >(pTarget)->IsInAVehicle() )
	{
		CBaseEntity *pVehicle = assert_cast< CPlayer * >(pTarget)->GetVehicleEntity();
		CEntity *cent_pVehicle = CEntity::Instance(pVehicle);
		cent_pVehicle->CollisionProp()->NormalizedToWorldSpace( Vector(0.0f,0.0f,1.0f), &vecMaxPos );
		cent_pVehicle->CollisionProp()->NormalizedToWorldSpace( Vector(0.0f,0.0f,0.0f), &vecMinPos );
...
void CNPC_Manhack::TranslateNavGoal( CBaseEntity *pEnemy, Vector &chasePosition )
{
	...
	if ( cent_pEnemy && cent_pEnemy->IsPlayer() && assert_cast< CPlayer * >(cent_pEnemy)->IsInAVehicle() )
	{
		Vector vecNewPos;
		CBaseEntity *pVehicle = assert_cast< CPlayer * >(cent_pEnemy)->GetVehicleEntity();
		CEntity *cent_pVehicle = CEntity::Instance(pVehicle);
		cent_pVehicle->CollisionProp()->NormalizedToWorldSpace( Vector(0.5,0.5,0.5f), &vecNewPos );
		chasePosition.z = vecNewPos.z;
	}
	else
	{
		Vector vecTarget;
		cent_pEnemy->CollisionProp()->NormalizedToCollisionSpace( Vector(0,0,0.75f), &vecTarget );
...
CBaseEntity *CNPC_Manhack::HasPhysicsAttacker( float dt )
{
	...
	{
		return (m_hPhysicsAttacker) ? m_hPhysicsAttacker->BaseEntity() : NULL;
	}
	return NULL;
}
...
void CNPC_Manhack::UpdatePanels( void )
{
	if ( m_flEngineStallTime > gpGlobals->curtime )
	{
		SetPoseParameter( m_iPanel1, enginerandom->RandomFloat( 0.0f, 90.0f ) );
		SetPoseParameter( m_iPanel2, enginerandom->RandomFloat( 0.0f, 90.0f ) );
		SetPoseParameter( m_iPanel3, enginerandom->RandomFloat( 0.0f, 90.0f ) );
		SetPoseParameter( m_iPanel4, enginerandom->RandomFloat( 0.0f, 90.0f ) );
		return;
	}
