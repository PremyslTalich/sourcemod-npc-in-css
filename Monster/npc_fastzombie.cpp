...
public:
	...
	void LeapAttackTouch( CEntity *pOther );
	void ClimbTouch( CEntity *pOther );
...
void CEFastZombie::PrescheduleThink( void )
{
 	...
 	if( m_pMoanSound && gpGlobals->curtime > m_flTimeUpdateSound )
	{
		...
		if( flDistNoBBox >= FASTZOMBIE_EXCITE_DIST && m_flDistFactor != 1.0 )
		{
			...
			g_SoundController->SoundChangePitch( m_pMoanSound, FASTZOMBIE_IDLE_PITCH, FASTZOMBIE_SOUND_UPDATE_FREQ );
		}
...
void CEFastZombie::SoundInit( void )
{
	if( !m_pMoanSound )
	{
		...
		g_SoundController->CommandClear( m_pMoanSound );
	}
	...
	{
		// Set up layer2
		m_pLayer2 = g_SoundController->SoundCreate( filter, entindex(), CHAN_VOICE, "NPC_FastZombie.Gurgle", ATTN_NORM );

		// Start silent.
		g_SoundController->Play( m_pLayer2, 0.0, 100 );
	}
...
void CEFastZombie::SetIdleSoundState( void )
{
	...
	if ( m_pMoanSound )
	{
		g_SoundController->SoundChangePitch( m_pMoanSound, FASTZOMBIE_IDLE_PITCH, 1.0 );
		g_SoundController->SoundChangeVolume( m_pMoanSound, 0.75, 1.0 );
	}

	// Second Layer
	if ( m_pLayer2 )
	{
		g_SoundController->SoundChangePitch( m_pLayer2, 100, 1.0 );
		g_SoundController->SoundChangeVolume( m_pLayer2, 0.0, 1.0 );
	}
}
...
void CEFastZombie::SetAngrySoundState( void )
{
	...
	// Main looping sound
	g_SoundController->SoundChangePitch( m_pMoanSound, FASTZOMBIE_MIN_PITCH, 0.5 );
	g_SoundController->SoundChangeVolume( m_pMoanSound, 1.0, 0.5 );
...
void CEFastZombie::PainSound( const CTakeDamageInfo &info )
{
	...
	if ( m_pMoanSound )
		g_SoundController->SoundPlayEnvelope( m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, envFastZombieInverseVolumePain, ARRAYSIZE(envFastZombieInverseVolumePain) );
}
...
int CEFastZombie::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	CPlayer *pPlayer = static_cast<CPlayer*>( GetEnemy() );

	if( pPlayer )
	{
		// If the enemy is a player, don't attack from behind!
		if( !pPlayer->FInViewCone_Entity( BaseEntity() ) )
		{
			return COND_NONE;
		}
	}
	...
	UTIL_TraceHull( WorldSpaceCenter(), WorldSpaceCenter() + vecDirToEnemy, vecHullMin, vecHullMax, MASK_NPCSOLID, BaseEntity(), COLLISION_GROUP_NONE, &tr );
...
void CEFastZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		ClawAttack( GetClawAttackRange(), sk_zombie_dmg_one_slash.GetInt(), viewPunch, right, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		QAngle viewPunch(-3.0f, -5.0f, -3.0f);
		right = right * 50;
		ClawAttack( GetClawAttackRange(), sk_zombie_dmg_one_slash.GetInt(), viewPunch, right, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}
...
void CEFastZombie::RunTask( const Task_t *pTask )
{
	...
	case TASK_RANGE_ATTACK1:
		if( ( GetFlags() & FL_ONGROUND ) || ( ce_m_pfnTouch == NULL ) )
		{
...
void CEFastZombie::ClimbTouch( CEntity *pOther )
{
	...
	else if ( dynamic_cast<CE_CPhysicsProp *>(pOther) )
	{
		g_helpfunc.NPCPhysics_CreateSolver( BaseEntity(), pOther->BaseEntity(), true, 5.0 );
	}
}
...
void CEFastZombie::StopLoopingSounds( void )
{
	...
	if ( m_pLayer2 )
	{
		g_SoundController->SoundDestroy( m_pLayer2 );
		m_pLayer2 = NULL;
	}
...
void CEFastZombie::OnChangeActivity( Activity NewActivity )
{
	...
	else if ( GetActivity() == ACT_GLIDE )
	{
		...
		if ( m_pMoanSound )
			g_SoundController->SoundChangePitch( m_pMoanSound, FASTZOMBIE_MIN_PITCH, 0.3 );
	}
	...
	else if ( GetActivity() == ACT_CLIMB_DISMOUNT || ( GetActivity() == ACT_CLIMB_UP && NewActivity != ACT_CLIMB_DISMOUNT ) )
	{
		// Ended a climb
		if ( m_pMoanSound )
			g_SoundController->SoundChangeVolume( m_pMoanSound, 1.0, 0.2 );
...
void CEFastZombie::BeginNavJump( void )
{
	...
	g_SoundController->SoundPlayEnvelope( m_pLayer2, SOUNDCTRL_CHANGE_VOLUME, envFastZombieVolumeJump, ARRAYSIZE(envFastZombieVolumeJump) );
}
