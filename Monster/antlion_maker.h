...
public:
	...
	CE_DECLARE_CLASS( CAntlionTemplateMaker, CTemplateNPCMaker );

	CAntlionTemplateMaker( void );
	~CAntlionTemplateMaker( void );

	void	MakeNPC( void );
	void	ChildPreSpawn( CAI_NPC *pChild );
	void	ChildPostSpawn( CAI_NPC *pChild );
	...
	void	SetFightTarget( string_t strTarget, CEntity *pActivator = NULL, CEntity *pCaller = NULL );
	void	SetFightTarget( CEntity *pEntity );
	void	SetFightTarget( const Vector &position );
	
	void	SetFollowTarget( string_t strTarget, CEntity *pActivator = NULL, CEntity *pCaller = NULL );
	void	SetFollowTarget( CEntity *pEntity );
	...
	CEntity	*GetFightTarget( void );
	CEntity *GetFollowTarget( void );
...
protected:
	...
	void		PrecacheTemplateEntity( CEntity *pEntity );

	bool		FindHintSpawnPosition( const Vector &origin, float radius, string_t hintGroupName, CE_AI_Hint **pHint, bool bRandom = false );
	bool		FindNearTargetSpawnPosition( Vector &origin, float radius, CEntity *pTarget );

	//These are used by FindNearTargetSpawnPosition
	bool		FindPositionOnFoot( Vector &origin, float radius, CEntity *pTarget );
	bool		FindPositionOnVehicle( Vector &origin, float radius, CEntity *pTarget );
	bool		ValidateSpawnPosition( Vector &vOrigin, CEntity *pTarget = NULL );
...
protected:
	...
	CFakeHandle		m_hFightTarget;		// A normal entity pointer for fight position
	CFakeHandle		m_hProxyTarget;		// This is a self-held target that is created and used when a vector is passed in as a fight
									// goal, instead of an entity
	CFakeHandle		m_hFollowTarget;	// Target to follow

	CUtlVector< CEFakeHandle< CNPC_Antlion > >	m_Children;
...
class CAntlionMakerManager : public CBaseGameSystem
{
public:
	CAntlionMakerManager( char const *name ) : CBaseGameSystem( name )
	{
	}
	...
	void	BroadcastFightGoal( CEntity *pFightGoal );
	void	BroadcastFollowGoal( CEntity *pFollowGoal );

protected:
	
	void	GatherMakers( void );

	CUtlVector< CEFakeHandle< CAntlionTemplateMaker > >	m_Makers;
};
