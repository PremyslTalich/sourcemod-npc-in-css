...
void CNPC_PoisonZombie::Spawn( void )
{
	...
	m_pFastBreathSound = g_SoundController->SoundCreate( filter, entindex(), CHAN_ITEM, "NPC_PoisonZombie.FastBreath", ATTN_IDLE );
	g_SoundController->Play( m_pFastBreathSound, 0.0f, 100 );

	CPASAttenuationFilter filter2( this );
	m_pSlowBreathSound = g_SoundController->SoundCreate( filter2, entindex(), CHAN_ITEM, "NPC_PoisonZombie.Moan1", ATTN_NORM );
	g_SoundController->Play( m_pSlowBreathSound, BREATH_VOL_MAX, 100 );
	...
	if (nCrabs == 1)
	{
		nBitMask = nBits[enginerandom->RandomInt( 0, 2 )];
	}
	else if (nCrabs == 2)
	{
		nBitMask = nBits[enginerandom->RandomInt( 3, 5 )];
	}
...
void CNPC_PoisonZombie::StopLoopingSounds( void )
{
	...
	g_SoundController->SoundDestroy( m_pSlowBreathSound );
...
void CNPC_PoisonZombie::BreatheOffShort( void )
{
	if ( m_bNearEnemy )
	{
		g_SoundController->SoundPlayEnvelope( m_pFastBreathSound, SOUNDCTRL_CHANGE_VOLUME, envPoisonZombieBreatheVolumeOffShort, ARRAYSIZE(envPoisonZombieBreatheVolumeOffShort) );
	}
	else
	{
		g_SoundController->SoundPlayEnvelope( m_pSlowBreathSound, SOUNDCTRL_CHANGE_VOLUME, envPoisonZombieBreatheVolumeOffShort, ARRAYSIZE(envPoisonZombieBreatheVolumeOffShort) );
	}
}
...
void CNPC_PoisonZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_POISON_THROW_CRAB )
	{
		...
		m_flNextCrabThrowTime = gpGlobals->curtime + enginerandom->RandomInt( ZOMBIE_THROW_MIN_DELAY, ZOMBIE_THROW_MAX_DELAY );
		return;
	}
...
int CNPC_PoisonZombie::RandomThrowCrab( void )
{
	...
	do
	{
		int nTest = enginerandom->RandomInt( 0, 2 );
		if ( m_bCrabs[nTest] )
		{
			nCrab = nTest;
		}
...
void CNPC_PoisonZombie::EvacuateNest( bool bExplosion, float flDamage, CBaseEntity *pAttacker )
{
	...
	for( int i = 0; i < MAX_CRABS ; i++ )
	{
		if( m_bCrabs[i] )
		{
			...
			vecAngles = QAngle( 0, enginerandom->RandomFloat( 0, 360 ), 0 );
			...
			pCrab->Eject( vecAngles, flVelocityScale, CEntity::Instance(pAttacker) );
			EnableCrab( i, false );
		}
	}
}
...
void CNPC_PoisonZombie::PrescheduleThink( void )
{
	...
	{
		m_flNextCrabThrowTime = gpGlobals->curtime + enginerandom->RandomInt( ZOMBIE_THROW_FIRST_MIN_DELAY, ZOMBIE_THROW_FIRST_MAX_DELAY );
	}
	...
	if ( bNearEnemy )
	{
		if ( !m_bNearEnemy )
		{
			...
			float duration = enginerandom->RandomFloat( 1.0f, 2.0f );
			g_SoundController->SoundChangeVolume( m_pFastBreathSound, BREATH_VOL_MAX, duration );
			g_SoundController->SoundChangePitch( m_pFastBreathSound, enginerandom->RandomInt( 100, 120 ), enginerandom->RandomFloat( 1.0f, 2.0f ) );

			g_SoundController->SoundChangeVolume( m_pSlowBreathSound, 0.0f, duration );
	...
	else if ( m_bNearEnemy )
	{
		...
		g_SoundController->SoundChangeVolume( m_pFastBreathSound, BREATH_VOL_MAX, duration );
		g_SoundController->SoundChangeVolume( m_pSlowBreathSound, 0.0f, duration );
//		g_SoundController->SoundChangePitch( m_pBreathSound, enginerandom->RandomInt( 80, 100 ), duration );
...
void CNPC_PoisonZombie::PainSound( const CTakeDamageInfo &info )
{
	...
	{	
		...
		m_flNextPainSoundTime = gpGlobals->curtime + enginerandom->RandomFloat( 4.0, 7.0 );
	}
}
...
void CNPC_PoisonZombie::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	if( !m_pMoanSound )
	{
		...
		m_flMoanPitch = enginerandom->RandomInt( 98, 110 );

		CPASAttenuationFilter filter( this, 1.5 );
		//m_pMoanSound = g_SoundController->SoundCreate( entindex(), CHAN_STATIC, pszSound, ATTN_NORM );
		m_pMoanSound = g_SoundController->SoundCreate( filter, entindex(), CHAN_STATIC, pszSound, 1.5 );

		g_SoundController->Play( m_pMoanSound, 0.5, m_flMoanPitch );
	}
	...
	float flPitchShift = enginerandom->RandomInt( -4, 4 );
	g_SoundController->SoundChangePitch( m_pMoanSound, m_flMoanPitch + flPitchShift, 0.3 );
