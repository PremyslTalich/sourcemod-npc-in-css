...
CNPC_Stalker::CNPC_Stalker()
{
	...
	m_fBeamEndTime = 0.0f;
	m_fBeamRechargeTime = 0.0f;
	m_fNextDamageTime = 0.0f;
	m_bPlayingHitWall = 0.0f;
	m_bPlayingHitFlesh = 0.0f;
	m_pBeam = NULL;
	m_pLightGlow = NULL;
	m_flNextNPCThink = 0.0f;
	m_flNextAttackSoundTime = 0.0f;
	m_flNextBreatheSoundTime = 0.0f;
	m_flNextScrambleSoundTime = 0.0f;
	m_iPlayerAggression = 0;
	m_flNextScreamTime = 0.0f;
}
...
int	CNPC_Stalker::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if (attacker->Classify() == CLASS_VORTIGAUNT)
	{
	...
	if ( (attacker->GetFlags() & FL_CLIENT) )
	{
...
void CNPC_Stalker::PrescheduleThink()
{
	if (gpGlobals->curtime > m_flNextBreatheSoundTime)
	{
		...
		m_flNextBreatheSoundTime = gpGlobals->curtime + 3.0 + enginerandom->RandomFloat( 0.0, 5.0 );
	}
}
...
bool CNPC_Stalker::IsValidEnemy( CBaseEntity *pEnemy )
{
	...
	Class_T enemyClass = cent_pEnemy->Classify();
	...
	if( enemyClass == CLASS_BULLSEYE && cent_pEnemy->GetParent() )
	{
		// This bullseye is in heirarchy with something. If that
		// something is held by the physcannon, this bullseye is 
		// NOT a valid enemy.
		IPhysicsObject *pPhys = cent_pEnemy->GetParent()->VPhysicsGetObject();
...
void CNPC_Stalker::Spawn( void )
{
	...
	m_iHealth			= sk_stalker_health.GetInt();
	...
	m_flNextBreatheSoundTime	= gpGlobals->curtime + enginerandom->RandomFloat( 0.0, 10.0 );
...
{
	CEntity *attacker = CEntity::Instance(info.GetAttacker());
	if( IsInSquad() && attacker->IsPlayer() )
	{
		AISquadIter_t iter;
		for ( CBaseEntity *pSquadMember = GetSquad()->GetFirstMember( &iter ); pSquadMember; pSquadMember = GetSquad()->GetNextMember( &iter ) )
		{
			...
			if ( cent_pSquadMember->IsAlive() && cent_pSquadMember != this )
			{
				CNPC_Stalker *pStalker = dynamic_cast <CNPC_Stalker*>(cent_pSquadMember);

				if( pStalker && pStalker->FVisible_Entity(info.GetAttacker()) )
				{
...
void CNPC_Stalker::UpdateAttackBeam( void )
{
	...
	if (pEnemy)
	{
		...
		{
			Vector offset = pEnemy->GetViewOffset();
			if(pEnemy->IsPlayer())
				offset.z -= 10;

			Vector enemyLKP = GetEnemyLKP();
			m_vLaserTargetPos = enemyLKP + offset;
			...
			trace_t tr;
			UTIL_TraceLine( vecSrc, vecSrc + m_vLaserDir * MAX_STALKER_FIRE_RANGE, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);
			// ---------------------------------------------
			...
			g_helpfunc.CSoundEnt_InsertSound(SOUND_DANGER, tr.endpos, 60, 0.025, BaseEntity());
		}
	}
	else
	{
...
void CNPC_Stalker::StartTask( const Task_t *pTask )
{
	...
	case TASK_STALKER_SCREAM:
	{
		if( gpGlobals->curtime > m_flNextScreamTime )
		{
			...
			m_flNextScreamTime = gpGlobals->curtime + enginerandom->RandomFloat( 10.0, 15.0 );
		}
	...
	case TASK_RANGE_ATTACK1:
		{
			...
			if (pEnemy)
			{
				...
				m_vLaserTargetPos = GetEnemyLKP() + offset;
				...
				{
					missPos.x += 60 + 120*enginerandom->RandomInt(-1,1);
					missPos.y += 60 + 120*enginerandom->RandomInt(-1,1);
				}
				else
				{
					missPos.x += 80*enginerandom->RandomInt(-1,1);
					missPos.y += 80*enginerandom->RandomInt(-1,1);
				}
				...
				// ----------------------------------------------------------------------
				CCombatCharacter *pBCC = ToBaseCombatCharacter(pEnemy);
				if (pBCC)
				{
...
void CNPC_Stalker::StartAttackBeam( void )
{
	...
	if (!m_pBeam)
	{
		...
		m_pBeam = CE_CBeam::BeamCreate( "sprites/laser.vmt", 2.0 );
		...
		switch (m_eBeamPower)
		{
			case STALKER_BEAM_LOW:
				...
				m_pLightGlow = CE_CSprite::SpriteCreate( "sprites/redglow1.vmt", GetAbsOrigin(), FALSE );
				break;
			case STALKER_BEAM_MED:
				m_pBeam->SetColor( 255, 50, 0 );
				m_pLightGlow = CE_CSprite::SpriteCreate( "sprites/orangeglow1.vmt", GetAbsOrigin(), FALSE );
				break;
			case STALKER_BEAM_HIGH:
				m_pBeam->SetColor( 255, 150, 0 );
				m_pLightGlow = CE_CSprite::SpriteCreate( "sprites/yellowglow1.vmt", GetAbsOrigin(), FALSE );
				break;
		}
		...
		m_pLightGlow->SetAttachment( BaseEntity(), 1 );
...
void CNPC_Stalker::DoSmokeEffect( const Vector &position )
{
	if ( gpGlobals->curtime > m_nextSmokeTime )
	{
		...
		UTIL_Smoke(position, enginerandom->RandomInt(5, 10), 10);
	}
}
...
void CNPC_Stalker::DrawAttackBeam(void)
{
	...
	Vector vv = vAttachPos;
	vv.z -= 8.0f;
	m_pBeam->SetEndPos(vv);
	...
	//  Play burn sounds
	// --------------------------------------------
	CCombatCharacter *pBCC = ToBaseCombatCharacter( CEntity::Instance(tr.m_pEnt) );
	if (pBCC && pBCC->Classify() != CLASS_STALKER)
	{
		if (gpGlobals->curtime > m_fNextDamageTime)
		{
			...
			CTakeDamageInfo info( BaseEntity(), BaseEntity(), damage, DMG_SHOCK );
...
bool CNPC_Stalker::InnateWeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
	...
	trace_t tr;
	UTIL_TraceLine( barrelPos, targetPos, MASK_SHOT, BaseEntity(), COLLISION_GROUP_NONE, &tr);
	...
	CEntity *pBE = CEntity::Instance(tr.m_pEnt);
	CCombatCharacter *pBCC = ToBaseCombatCharacter( pBE );
	if ( pBE == GetEnemy() )
	{
		return true;
	}
	...
	{
		if (IRelationType( pBCC->BaseEntity() ) == D_HT)
		{
			return true;
		}
...
void CNPC_Stalker::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		{			
			CBaseEntity *cbase = CheckTraceHullAttack_Float( 32, Vector(-16,-16,-16), Vector(16,16,16), sk_stalker_melee_dmg.GetInt(), DMG_SLASH );
			...
			if ( pHurt )
			{
				...
				{
					pHurt->ViewPunch( QAngle( 5, 0, enginerandom->RandomInt(-10,10)) );
				}
				
				// Spawn some extra blood if we hit a BCC
				CCombatCharacter* pBCC = ToBaseCombatCharacter( pHurt );
				if (pBCC)
				{
...
void CNPC_Stalker::AddZigZagToPath(void) 
{
	...
	if (waypointDir.LengthSqr() > ZIG_ZAG_SIZE)
	{
		...
		float distance = enginerandom->RandomFloat( 30, 60 );
